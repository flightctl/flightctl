...
- **Connection pooling support** (FlightCtl uses up to 100 max open connections, 10 max idle connections)
...
# Optional: only needed if FlightCtl should create users automatically
...
```bash
# Deploy FlightCtl (will automatically use external database based on service-config.yaml)
