...
### 3.2 Download your pull secret.

You need to download your pull secret in this directory as `auths.json`, and if you're using a downstream ACM, you also need to add `quay.io:443` registry in your pull secret.

### 3.3 Download flightctl config file.
...
### 3.6 Approve the device.

When you approve a device, make sure you set the label `fleet:acm`. Afther the device is approved in `Edege Management`, you will see a same name cluster registered and automatically approved in ACM.
