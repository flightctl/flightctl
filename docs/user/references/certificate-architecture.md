...
Generation: `global.generateCertificates` (`auto`, `cert-manager`, `builtin`, `none`)
...
| `/etc/flightctl/pki/flightctl-alertmanager-proxy/server.crt` | Alertmanager proxy cert |

Certificates are auto-generated on first start. To use your own CA, see [Custom Certificates](../installing/installing-service-on-linux.md#custom-certificates).
