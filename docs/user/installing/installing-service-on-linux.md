...
For a production environment, you want to set the base domain (`global.baseDomain`) to your own fully qualified domain name (FQDN) and configure your own certificates (see [Custom Certificates](#custom-certificates)).

When you are done, start the Flight Control services by running
...
Once the UI service has spun up, visit the UI at `https://BASE_DOMAIN` (where `BASE_DOMAIN` is your configured base domain).
