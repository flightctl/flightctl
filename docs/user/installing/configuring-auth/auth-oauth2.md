...
- **Per User** (`type: perUser`): Creates a separate organization for each user
  ...
  **Important:** When choosing `organizationAssignment=perUser`, you must use `roleAssignment=static` and select only `flightctl-org-admin` or `flightctl-admin` roles. Since each user manages their own organization, only these admin-level roles provide the necessary permissions. See below for details on role assignment.
