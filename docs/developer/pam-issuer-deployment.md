...
1. **flightctl-pam-issuer-deployment.yaml**
   ...
   - Depends on database, KV store, and migration services
...
global:
  ...
  auth:
    pamOidcIssuer:
      issuer: ""  # Base URL for the OIDC issuer (defaults to API server URL)
...
- **Database**: Shares the same database as the main API server for session management
- **KV Store**: Uses Redis for session storage
- **Authentication**: 
...
```
https://pam-issuer.<baseDomain>/api/v1/auth/.well-known/openid-configuration
```
...
auth:
  oidc:
    - clientId: flightctl-client
      enabled: true
      issuer: https://pam-issuer.<baseDomain>
