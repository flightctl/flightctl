...
| Profile | Typical use |
|---|---|
| `backend-internal` | Worker-style services (Helm + quadlet, internal namespace, ServiceAccount) |
...
- Helm under `deploy/helm/flightctl/templates/<name>/` (Deployment; ServiceAccount; for external also Route + Service)
...
- Helm values/schema/templates dir/deployment; ServiceAccount; for external also Route + Service; `helm-chart-opts` image keys
