...
**Session Storage**: Redis (shared with main API server)

**Database**: PostgreSQL (shared with main API server for user/group lookups)

### Service Dependencies

The PAM issuer service has the following dependencies (defined in `flightctl-pam-issuer.container`):
```
After=flightctl-db-wait.service flightctl-db-migrate.service flightctl-kv.service flightctl-pam-issuer-init.service
Wants=flightctl-db-wait.service flightctl-db-migrate.service flightctl-kv.service flightctl-pam-issuer-init.service
```

This ensures:
1. Database is ready and migrated
2. Redis is available
3. Configuration is templated correctly
...
helm upgrade --install flightctl ./deploy/helm/flightctl \
  ...
  --set global.auth.pamOidcIssuer.issuer=https://pam-issuer.example.com \
...
global:
  auth:
    pamOidcIssuer:
      issuer: https://pam-issuer.example.com
      clientId: flightctl-client
      clientSecret: <your-secret>
      scopes: ["openid", "profile", "email", "roles"]
      redirectUris: ["https://ui.example.com/auth/callback"]
      pamService: flightctl
```
...
3. **Session Storage**: Sessions are stored in Redis with expiration
