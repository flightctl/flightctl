...
func createAgents(log *logrus.Logger, numDevices int, initialDeviceIndex int, agentConfigTemplate *agent_config.Config) ([]*agent.Agent, []string) {
	...
	for i := 0; i < numDevices; i++ {
		...
		cfg.TPMConfig.Path = ""
		cfg.TPMConfig.PersistenceType = agentConfigTemplate.TPMConfig.PersistenceType
		cfg.TPMConfig.PersistenceMetadata = agentConfigTemplate.TPMConfig.PersistenceMetadata
...
func setupTPMLinks(agentDir string, log *logrus.Logger) {
	...
	log.Infof("sys class dirs created")
	...
	if linkCount > 0 {
		log.Infof("Created %d TPM device links for agent %s", linkCount, filepath.Base(agentDir))
	} else {
		log.Infof("No TPM device links created for agent %s", filepath.Base(agentDir))
	}
}
