...
func copyAgentFiles(log *logrus.Logger, certDir, agentDir string) {
	...
	fileCopyMap := map[string]struct {
		...
		certDir: {
			dir:   copyDir(agent_config.DefaultConfigDir),
			files: []string{"ca.crt", "client-enrollment.crt", "client-enrollment.key"},
		},
		// copies the routing tables from /proc/net into the agent's directories so that it will be able to parse
		// them and select a default route. Currently, it spends 45 seconds attempting to resolve a default route
		// before giving up and continuing on with the normal startup
		"/proc/net": {
			dir:   copyDir("/proc/net"),
			files: []string{"route", "ipv6_route"},
		},
	}
	for dir, info := range fileCopyMap {
		for _, filename := range info.files {
			if err := copyFile(filepath.Join(dir, filename), filepath.Join(info.dir, filename)); err != nil {
...
func createAgents(agentCfg createAgentsConfig) ([]*agent.Agent, []string) {
	...
	for i := 0; i < agentCfg.numDevices; i++ {
		...
		cfg.TPM.Enabled = agentCfg.agentConfigTemplate.TPM.Enabled
		cfg.TPM.Path = agentCfg.agentConfigTemplate.TPM.Path
		cfg.TPM.PersistencePath = agentCfg.agentConfigTemplate.TPM.PersistencePath
