...
func runCmd(log *logrus.Logger) error {
	...
	services, err := setupServices(ctx, log, apiCfg, &cleanupFuncs)
	if err != nil {
		return err
	}

	// Setup and run servers
	return runServers(ctx, log, apiCfg, services)
}
...
func runServers(ctx context.Context, log *logrus.Logger, apiCfg *apiConfig, services *apiServices) error {
	...
	go func() {
		...
		errCh <- fmt.Errorf("API server: %w", server.Run(ctx))
	}()

	go func() {
		log.Info("Starting agent server")
		errCh <- fmt.Errorf("agent server: %w", agentServer.Run(ctx))
	}()

	setupMetricsServer(ctx, log, apiCfg.cfg, services, errCh)

	// Wait for any server to complete or signal
...
func setupMetricsServer(ctx context.Context, log *logrus.Logger, cfg *config.Config, services *apiServices, errCh chan error) {
	...
	if len(collectors) > 0 {
		go func() {
			...
			errCh <- fmt.Errorf("metrics server: %w", tracing.RunMetricsServer(ctx, log, cfg.Metrics.Address, collectors...))
		}()
	}
}
