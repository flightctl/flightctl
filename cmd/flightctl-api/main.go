...
func main() {
	...
	ca, fresh, err := crypto.EnsureCA(cfg.CA)
	if fresh {
	...
	if cfg.Service.SrvCertFile != "" || cfg.Service.SrvKeyFile != "" {
		...
		if canReadCertAndKey, _ := crypto.CanReadCertAndKey(srvCertFile, srvKeyFile); canReadCertAndKey {
			...
			serverCerts, err = ca.MakeAndWriteServerCertificate(srvCertFile, srvKeyFile, cfg.Service.AltNames, cfg.CA.ServerCertValidityDays*3600*24)
			if err != nil {
