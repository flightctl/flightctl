...
func main() {
	...
	agentServerInstance := agentserver.New(log, cfg, store, ca, agentListener, provider, agentTlsConfig, orgResolver)
	...
	go func() {
		...
		server := apiserver.New(log, cfg, store, ca, listener, provider, agentServerInstance.GetGRPCServer(), orgResolver)
	...
	go func() {
		if err := agentServerInstance.Run(ctx); err != nil {
			log.Fatalf("Error running server: %s", err)
		}
		cancel()
	}()
