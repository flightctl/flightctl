...
func main() {
	...
	ca, fresh, err := crypto.EnsureCA(cfg.CAConfig)
	if fresh {
		log.Info("CA has been autogenerated")
	}
	if err != nil {
	...
	if cfg.Service.SrvCertFile != "" || cfg.Service.SrvKeyFile != "" {
		...
		if canReadCertAndKey, _ := crypto.CanReadCertAndKey(srvCertFile, srvKeyFile); canReadCertAndKey {
			...
			serverCerts, err = ca.MakeAndWriteServerCertificate(srvCertFile, srvKeyFile, cfg.Service.AltNames, cfg.CAConfig.ServerCertValidityDays*3600*24)
			if err != nil {
	...
	_, _, err = ca.EnsureClientCertificate(cfg.CAConfig.ClientBootstrapCertName, cfg.CAConfig.ClientBootstrapCertName, cfg.CAConfig.ClientBootstrapCommonName, cfg.CAConfig.ClientBootstrapValidityDays)
	if err != nil {
