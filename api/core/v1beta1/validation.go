...
func (ci CatalogItem) Validate() []error {
	...
	for i, version := range ci.Spec.Versions {
		allErrs = append(allErrs, validateCatalogItemVersion(version, i, seenVersions)...)
	}
	...
	if category != CatalogItemCategorySystem && category != CatalogItemCategoryApplication && category != CatalogItemCategoryAsset {
		allErrs = append(allErrs, fmt.Errorf("spec.category must be %q, %q, or %q", CatalogItemCategorySystem, CatalogItemCategoryApplication, CatalogItemCategoryAsset))
	}
...
func validateCatalogItemReference(ref *CatalogItemReference) []error {
	...
	if ref.Related != nil {
		relatedCount := len(*ref.Related)
		seenTypes := make(map[CatalogItemArtifactType]struct{})
		for i, related := range *ref.Related {
			// type is required if more than 1 related item, defaults to "container" if only 1
			relatedType := CatalogItemArtifactType("")
			if related.Type != nil {
				relatedType = *related.Type
			}
			if relatedType == "" {
				if relatedCount > 1 {
					allErrs = append(allErrs, fmt.Errorf("spec.reference.related[%d].type is required when multiple related items exist", i))
				}
				relatedType = CatalogItemArtifactTypeContainer // default for single item
			}
			if relatedType != "" {
				if _, exists := seenTypes[relatedType]; exists {
					allErrs = append(allErrs, fmt.Errorf("spec.reference.related[%d].type: duplicate type %q", i, relatedType))
				}
				seenTypes[relatedType] = struct{}{}
			}
			if related.Uri == "" {
				allErrs = append(allErrs, fmt.Errorf("spec.reference.related[%d].uri is required", i))
			} else {
				allErrs = append(allErrs, validateArtifactURI(related.Uri, fmt.Sprintf("spec.reference.related[%d].uri", i))...)
			}
		}
	}
...
func validateCatalogItemVersion(version CatalogItemVersion, index int, seenVersions map[string]struct{}) []error {
	allErrs := []error{}
...
func isValidCatalogItemType(t CatalogItemType) bool {
	switch t {
	case CatalogItemTypeOS, CatalogItemTypeFirmware,
...
func validCatalogItemTypes() string {
	return "os, firmware, container, helm, quadlet, compose, data"
}

// validateCategoryTypeCompatibility ensures the category and type are compatible.
// - system: os, firmware
// - application: container, helm, quadlet, compose
// - asset: data
func validateCategoryTypeCompatibility(category CatalogItemCategory, itemType CatalogItemType) error {
	...
	case CatalogItemCategorySystem:
		if itemType != CatalogItemTypeOS && itemType != CatalogItemTypeFirmware {
			return fmt.Errorf("spec.type %q is not valid for category %q; must be %q or %q",
				itemType, category, CatalogItemTypeOS, CatalogItemTypeFirmware)
		}
	case CatalogItemCategoryApplication:
		if itemType != CatalogItemTypeContainer && itemType != CatalogItemTypeHelm &&
			...
			return fmt.Errorf("spec.type %q is not valid for category %q; must be %q, %q, %q, or %q",
				itemType, category, CatalogItemTypeContainer, CatalogItemTypeHelm,
				CatalogItemTypeQuadlet, CatalogItemTypeCompose)
		}
	case CatalogItemCategoryAsset:
		if itemType != CatalogItemTypeData {
			return fmt.Errorf("spec.type %q is not valid for category %q; must be %q",
				itemType, category, CatalogItemTypeData)
		}
	}
	return nil
}
...
func validateCatalogItemConfig(category CatalogItemCategory, itemType CatalogItemType, config *map[string]interface{}, pathPrefix string) []error {
	...
	case CatalogItemCategoryAsset:
		allErrs = append(allErrs, validateAssetConfig(*config, configPath)...)
	}

	return allErrs
}

// Asset category known fields
var assetKnownFields = []string{"mountPath"}

// validateAssetConfig validates config for asset category items.
func validateAssetConfig(config map[string]interface{}, pathPrefix string) []error {
	allErrs := []error{}
	allErrs = append(allErrs, validateKnownFields(config, pathPrefix, assetKnownFields)...)
	return allErrs
}
...
// Uses the jsonschema library to validate the schema is syntactically correct
// and follows the JSON Schema draft-07 specification.
