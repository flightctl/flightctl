...
func ValidateContainerImageApplicationSpec(appName string, spec *ImageApplicationProviderSpec) []error {
	...
	if spec.Resources != nil && spec.Resources.Limits != nil {
		errs = append(errs, validateCPULimit(spec.Resources.Limits.Cpu, fmt.Sprintf("spec.applications[%s].resources.limits.cpu", appName))...)
		errs = append(errs, validateMemoryLimit(spec.Resources.Limits.Memory, fmt.Sprintf("spec.applications[%s].resources.limits.memory", appName))...)
	}
	return errs
}
...
func validateCPULimit(cpu *string, path string) []error {
	if cpu == nil {
		return nil
	}
	...
	if err != nil {
		return []error{fmt.Errorf("%s: must be a valid number, got %q", path, *cpu)}
	}
	return nil
}
...
func validateMemoryLimit(memory *string, path string) []error {
	if memory == nil {
		return nil
	}
