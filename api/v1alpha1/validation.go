...
func (o OAuth2ProviderSpec) Validate(ctx context.Context) []error {
	...
	if o.AuthorizationUrl == "" {
		allErrs = append(allErrs, errors.New("authorizationUrl is required"))
	}
	if o.TokenUrl == "" {
		allErrs = append(allErrs, errors.New("tokenUrl is required"))
	}
	if o.UserinfoUrl == "" {
		allErrs = append(allErrs, errors.New("userinfoUrl is required"))
	}
	if o.ClientId == "" {
		allErrs = append(allErrs, errors.New("clientId is required"))
	}
...
func (a AuthStaticOrganizationAssignment) Validate(ctx context.Context) []error {
	...
	if mappedIdentityVal == nil {
		allErrs = append(allErrs, errors.New("mapped identity not found in context"))
		return allErrs
	}

	mappedIdentity, ok := mappedIdentityVal.(*identity.MappedIdentity)
	if !ok {
		allErrs = append(allErrs, errors.New("invalid mapped identity type in context"))
		return allErrs
	}
...
func (a AuthDynamicOrganizationAssignment) Validate(ctx context.Context) []error {
	...
	if !hasAdminRole {
		allErrs = append(allErrs, errors.New("only flightctl-admin users are allowed to create auth providers with dynamic organization mapping"))
	}
...
func (a AuthPerUserOrganizationAssignment) Validate(ctx context.Context) []error {
	...
	if !hasAdminRole {
		allErrs = append(allErrs, errors.New("only flightctl-admin users are allowed to create auth providers with per-user organization mapping"))
	}
...
func (a AuthStaticRoleAssignment) Validate(ctx context.Context) []error {
	...
	if len(a.Roles) == 0 {
		allErrs = append(allErrs, errors.New("at least one role is required for static role assignment"))
	}
...
func (a AuthDynamicRoleAssignment) Validate(ctx context.Context) []error {
	...
	if len(a.ClaimPath) == 0 {
		allErrs = append(allErrs, errors.New("claimPath is required for dynamic role assignment"))
	}
