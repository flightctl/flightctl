...
type allowedPropertiesDevice struct {
	Metadata ObjectMeta
}
...
func validateParametersInString(s *string, path string, fleetTemplate bool) (bool, []error) {
	...
	if err != nil {
		var missingKeyError *MissingKeyGetOrError
		// Ignore errors returned by getOrError, because we're running with empty labels
		if !errors.As(err, &missingKeyError) {
