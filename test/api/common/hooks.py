...
# so we access the class from the registry.
...
_TracecovHandler = CUSTOM_HANDLERS[-1]
...
def apply_label_constraints(schema):
    ...
    for key in ("items", "additionalProperties", "not"):
        if key in schema:
            apply_label_constraints(schema[key])

    for key in ("properties", "patternProperties"):
        if key in schema and isinstance(schema[key], dict):
            for prop_schema in schema[key].values():
                apply_label_constraints(prop_schema)

    for key in ("allOf", "anyOf", "oneOf", "prefixItems"):
        if key in schema and isinstance(schema[key], list):
            for subschema in schema[key]:
                apply_label_constraints(subschema)

    if "x-bundled" in schema and isinstance(schema["x-bundled"], dict):
        for bundled_schema in schema["x-bundled"].values():
            apply_label_constraints(bundled_schema)
...
def fix_post_body(ctx, case, **kwargs):
    ...
    if not kind:
        return
    expected_api_version = f"{API_GROUP}/{VERSION}"
    if case.body.get("apiVersion") == expected_api_version:
        return
    for fn, path_regex, methods, _order in _BODY_MUTATORS:
        if path_regex and not re.search(path_regex, path):
            continue
