...
var _ = Describe("Device LastSeen Integration Tests", func() {
	...
	Describe("GetDeviceLastSeen", func() {
		...
		It("should return lastSeen timestamp for device with last seen timestamp", func() {
			...
			// Update the device status with a last seen timestamp
			now := time.Now()
			device.Status = &api.DeviceStatus{
				LastSeen: lo.ToPtr(now),
			}

			// Update the device status
			_, status = suite.Handler.ReplaceDeviceStatus(suite.Ctx, deviceName, device)
			Expect(status.Code).To(Equal(int32(200)))
	...
	Context("Device field selector tests", func() {
		It("should filter devices by lastSeen field selector", func() {
			...
			// Update device1 with recent lastSeen
			device1.Status = &api.DeviceStatus{
				LastSeen: lo.ToPtr(recentTime),
			}
			_, status = suite.Handler.ReplaceDeviceStatus(ctx, device1Name, device1)
			Expect(status.Code).To(Equal(int32(200)))
			...
			// Update device2 with old lastSeen
			device2.Status = &api.DeviceStatus{
				LastSeen: lo.ToPtr(oldTime),
			}
			_, status = suite.Handler.ReplaceDeviceStatus(ctx, device2Name, device2)
			Expect(status.Code).To(Equal(int32(200)))
