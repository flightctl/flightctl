...
var _ = Describe("Catalog Integration Tests", func() {
	...
	Context("CatalogItem category and type validation", func() {
		...
		It("should accept asset category with data type", func() {
			item := createValidCatalogItem("asset-data")
			item.Spec.Category = lo.ToPtr(api.CatalogItemCategoryAsset)
		...
		It("should reject asset category with helm type", func() {
			item := createValidCatalogItem("asset-helm-invalid")
			item.Spec.Category = lo.ToPtr(api.CatalogItemCategoryAsset)
			item.Spec.Type = api.CatalogItemTypeHelm
	...
	Context("CatalogItem related artifacts validation", func() {
		...
		It("should accept valid related artifacts", func() {
			item := createValidCatalogItem("valid-related")
			item.Spec.Category = lo.ToPtr(api.CatalogItemCategorySystem)
			item.Spec.Type = api.CatalogItemTypeOS
			item.Spec.Reference = api.CatalogItemReference{
				Uri: "quay.io/redhat/rhel-bootc",
				Related: &[]api.CatalogItemRelatedReference{
		...
		It("should reject related artifact missing uri", func() {
			item := createValidCatalogItem("related-missing-uri")
			item.Spec.Category = lo.ToPtr(api.CatalogItemCategorySystem)
			item.Spec.Type = api.CatalogItemTypeOS
			item.Spec.Reference = api.CatalogItemReference{
				Uri: "quay.io/redhat/rhel-bootc",
		...
		It("should reject multiple related artifacts missing type", func() {
			item := createValidCatalogItem("related-missing-type")
			item.Spec.Category = lo.ToPtr(api.CatalogItemCategorySystem)
			item.Spec.Type = api.CatalogItemTypeOS
			item.Spec.Reference = api.CatalogItemReference{
				Uri: "quay.io/redhat/rhel-bootc",
