...
var _ = Describe("Delta prepare deadline poll", func() {
	var (
		...
		dbName          string
		db              *gorm.DB
		prepareStore    *deltapreparestore.PrepareStore
		generationStore deltastore.Store
		prepareSvc      deltaprepareservice.Service
	...
	BeforeEach(func() {
		...
		prepareSvc = deltaprepareservice.WrapWithTracing(deltaprepareservice.NewServiceHandler(prepareStore, nil))
	...
	It("should fail a past-deadline waiting prepare and emit FleetRolloutStarted without changing generation status", func() {
		...
		_, err := generationStore.InsertDeltaGenerations(ctx, []*model.DeltaGeneration{{
			OrgID:           orgId,
		...
		Expect(prepareStore.CreateDeltaPrepare(ctx, prep)).To(Succeed())
		...
		gen, err := generationStore.GetDeltaGeneration(ctx, key)
		Expect(err).ToNot(HaveOccurred())
