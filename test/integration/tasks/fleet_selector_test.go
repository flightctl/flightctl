...
var _ = Describe("FleetSelector", func() {
	...
	validateDeviceOwnershipChangedEvent := func(deviceName string, events []api.Event, expectedPreviousOwner, expectedNewOwner *string) {
		found := false
		...
		Expect(found).To(BeTrue(), fmt.Sprintf("DeviceOwnershipChanged event not found for device %s", deviceName))
	}
	...
	Context("FleetSelector", func() {
		It("Fleet selector with event validation", func() {
			...
			for _, device := range devices.Items {
				...
				case "no-owner":
					...
					validateDeviceOwnershipChangedEvent("no-owner", events, nil, lo.ToPtr("fleet"))
				...
				case "otherfleet-to-fleet":
					...
					validateDeviceOwnershipChangedEvent("otherfleet-to-fleet", events, lo.ToPtr("otherfleet"), lo.ToPtr("fleet"))
				case "fleet-to-none":
					Expect(device.Metadata.Owner).To(BeNil())
					events := getEventsForResource(api.DeviceKind, "fleet-to-none")
					validateDeviceOwnershipChangedEvent("fleet-to-none", events, lo.ToPtr("fleet"), nil)
				case "stay-in-fleet":
					Expect(*device.Metadata.Owner).To(Equal("Fleet/fleet"))
		...
		It("Fleet deleted should remove device owners and emit events", func() {
			...
			validateDeviceOwnershipChangedEvent("device", events, lo.ToPtr("fleet"), nil)
		...
		It("Nil fleet selector should match no devices and emit events", func() {
			...
			for _, device := range devices.Items {
				...
				validateDeviceOwnershipChangedEvent(*device.Metadata.Name, events, lo.ToPtr("fleet"), nil)
			}
		...
		It("Fleet selector updated with multiple owners resolves conflicts and emits events", func() {
			...
			for _, device := range devices.Items {
				...
				case "fleet":
					...
					validateDeviceOwnershipChangedEvent("fleet", events, lo.ToPtr("fleet2"), lo.ToPtr("fleet"))
				...
				case "fleet2":
					...
					Expect(api.IsStatusConditionTrue(device.Status.Conditions, api.ConditionTypeDeviceMultipleOwners)).To(BeFalse())
					validateDeviceOwnershipChangedEvent("fleet2", events, lo.ToPtr("fleet"), lo.ToPtr("fleet2"))
				...
				case "fleet3":
					...
					Expect(api.IsStatusConditionTrue(device.Status.Conditions, api.ConditionTypeDeviceMultipleOwners)).To(BeFalse())
					validateDeviceOwnershipChangedEvent("fleet3", events, lo.ToPtr("fleet2"), lo.ToPtr("fleet3"))
				...
				case "nofleet":
					Expect(device.Metadata.Owner).To(BeNil())
					Expect(api.IsStatusConditionTrue(device.Status.Conditions, api.ConditionTypeDeviceMultipleOwners)).To(BeFalse())
					validateDeviceOwnershipChangedEvent("nofleet", events, lo.ToPtr("fleet4"), nil)
				...
				case "nolabels":
					Expect(device.Metadata.Owner).To(BeNil())
					Expect(api.IsStatusConditionTrue(device.Status.Conditions, api.ConditionTypeDeviceMultipleOwners)).To(BeFalse())
					validateDeviceOwnershipChangedEvent("nolabels", events, lo.ToPtr("fleet4"), nil)
		...
		It("Device labels updated with comprehensive event validation", func() {
			...
			for _, device := range devices.Items {
				...
				case "change-to-fleet2":
					...
					Expect(api.IsStatusConditionTrue(updatedDev.Status.Conditions, api.ConditionTypeDeviceMultipleOwners)).To(BeFalse())
					validateDeviceOwnershipChangedEvent(*device.Metadata.Name, events, lo.ToPtr("fleet1"), lo.ToPtr("fleet2"))
				...
				case "no-match":
					...
					Expect(api.IsStatusConditionTrue(updatedDev.Status.Conditions, api.ConditionTypeDeviceMultipleOwners)).To(BeFalse())
					validateDeviceOwnershipChangedEvent(*device.Metadata.Name, events, lo.ToPtr("fleet2"), nil)
				case "no-labels":
					Expect(updatedDev.Metadata.Owner).To(BeNil())
					Expect(api.IsStatusConditionTrue(updatedDev.Status.Conditions, api.ConditionTypeDeviceMultipleOwners)).To(BeFalse())
					validateDeviceOwnershipChangedEvent(*device.Metadata.Name, events, lo.ToPtr("fleet3"), nil)
				}
			}
		})
		...
		It("Should handle device with no labels in DeviceLabelsUpdated", func() {
			...
			validateDeviceOwnershipChangedEvent("no-labels-device", events, lo.ToPtr("fleet1"), nil)
		})
		...
		It("Should handle device with empty labels in DeviceLabelsUpdated", func() {
			...
			validateDeviceOwnershipChangedEvent("empty-labels-device", events, lo.ToPtr("fleet1"), nil)
		})
