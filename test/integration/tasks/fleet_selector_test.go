...
var _ = Describe("FleetSelector", func() {
	...
	Context("FleetSelector", func() {
		It("Fleet selector updated no overlap", func() {
			...
			for _, device := range devices.Items {
				...
				case "otherfleet-to-error":
					...
					Expect(api.IsStatusConditionTrue(device.Status.Conditions, api.ConditionTypeMultipleOwners)).To(BeTrue())
				}
			}
			...
			for _, fleet := range fleets.Items {
				cond := api.IsStatusConditionTrue(fleet.Status.Conditions, api.ConditionTypeOverlappingSelectors)
		...
		It("Fleet selector updated with overlap", func() {
			...
			condition := api.Condition{Type: api.ConditionTypeOverlappingSelectors, Status: api.ConditionStatusTrue}
			listParams := store.ListParams{Limit: 0}
			fleets, err := fleetStore.List(ctx, orgId, listParams)
			Expect(err).ToNot(HaveOccurred())
			...
			for _, device := range devices.Items {
				condition := api.Condition{Type: api.ConditionTypeMultipleOwners, Status: api.ConditionStatusTrue, Message: "overlap"}
			...
			Expect(len(fleets.Items)).To(Equal(3))
			for _, fleet := range fleets.Items {
				condTrue := api.IsStatusConditionTrue(fleet.Status.Conditions, api.ConditionTypeOverlappingSelectors)
			...
			for _, device := range devices.Items {
				...
				case "fleet":
					Expect(*device.Metadata.Owner).To(Equal("Fleet/fleet"))
					Expect(api.IsStatusConditionTrue(device.Status.Conditions, api.ConditionTypeMultipleOwners)).To(BeFalse())
				case "fleet2":
		...
		It("Device labels updated with no overlap", func() {
			...
			for _, device := range devices.Items {
				...
				case "stay-with-fleet1":
					...
					Expect(api.IsStatusConditionTrue(updatedDev.Status.Conditions, api.ConditionTypeMultipleOwners)).To(BeFalse())
				case "change-to-fleet2":
					Expect(*updatedDev.Metadata.Owner).To(Equal("Fleet/fleet2"))
					Expect(api.IsStatusConditionTrue(updatedDev.Status.Conditions, api.ConditionTypeMultipleOwners)).To(BeFalse())
				case "multiple-owners":
					Expect(*updatedDev.Metadata.Owner).To(Equal("Fleet/fleet1"))
					Expect(api.IsStatusConditionTrue(updatedDev.Status.Conditions, api.ConditionTypeMultipleOwners)).To(BeTrue())
				case "no-match":
		...
		It("Delete all fleets", func() {
			...
			condition := api.Condition{Type: api.ConditionTypeMultipleOwners, Status: api.ConditionStatusTrue}
