...
var _ = Describe("PrepareDeltas persist", func() {
	...
	When("two eligible devices share a digest pair", func() {
		It("should insert one generation and enqueue one GenerateDelta", func() {
			...
			progressService := deltapreparegeneration.NewProgressHandler(deltaPrepareGenerationStore, prepareService, nil)
			generationService := deltageneration.NewServiceHandler(deltaGenerationStore, progressService.EmitForGeneration, log)
