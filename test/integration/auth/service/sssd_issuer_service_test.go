...
func TestSSSDIssuerServiceIntegration(t *testing.T) {
	RegisterFailHandler(Fail)
	RunSpecs(t, "SSSD Issuer Service Integration Suite")
}

var _ = Describe("SSSD Issuer Service Integration Tests", func() {
	var (
	...
	BeforeEach(func() {
		...
		// Create SSSD issuer with real components
		sssdConfig := &config.SSSDOIDCIssuer{
		...
		// Create a mock SSSD authenticator that accepts a single test user
		mockAuthenticator := &MockSSSDAuthenticator{
			validUser:     "testuser",
			validPassword: "testpass",
		}

		provider, err = issuer.NewSSSDOIDCProviderWithAuthenticator(caClient, sssdConfig, mockAuthenticator)
		Expect(err).ToNot(HaveOccurred())
	...
	Context("Complete OIDC Flow with Real Auth Code", func() {
		It("should handle the complete flow with a real authorization code", func() {
			...
			By("Step 2: Simulating login with mock SSSD authenticator")
			...
			// The system should now return the actual groups from SSSD as roles
...
// MockSSSDAuthenticator is a mock implementation of SSSDAuthenticator for testing
type MockSSSDAuthenticator struct {
	validUser     string
	validPassword string
}

// Authenticate implements the SSSDAuthenticator interface
...
// LookupUser implements the SSSDAuthenticator interface
...
// GetUserGroups implements the SSSDAuthenticator interface
...
// Close implements the SSSDAuthenticator interface
func (m *MockSSSDAuthenticator) Close() error {
	return nil
}
