...
var _ = Describe("DeviceStore create", func() {
	...
	Context("Device store", func() {
		...
		It("When UpdateStatus sets LastUpdateFallbackReason and Updated.Size it should return them on Get", func() {
			name := "os-delta-status-fields"
			testutil.CreateTestDevice(ctx, devStore, orgId, name, nil, nil, nil)
			status := api.NewDeviceStatus()
			status.Os.LastUpdateFallbackReason = lo.ToPtr("delta apply failed")
			status.Updated.Size = lo.ToPtr("45 MiB")
			device := api.Device{
				Metadata: api.ObjectMeta{Name: lo.ToPtr(name)},
				Status:   &status,
			}
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(got.Status).ToNot(BeNil())
			Expect(got.Status.Os.LastUpdateFallbackReason).ToNot(BeNil())
			Expect(*got.Status.Os.LastUpdateFallbackReason).To(Equal("delta apply failed"))
			Expect(got.Status.Updated.Size).ToNot(BeNil())
			Expect(*got.Status.Updated.Size).To(Equal("45 MiB"))
		})
