...
var _ = Describe("Encryption migration", func() {
	...
	It("When Device rendered fields are plaintext it should migrate them to the active key", func() {
		Expect(db.WithContext(ctx).Exec(
			`INSERT INTO devices (org_id, name, generation, resource_version, created_at, updated_at, rendered_config, rendered_applications)
			 VALUES (?, ?, 1, 1, NOW(), NOW(), ?::jsonb, ?::jsonb)`,
			orgId, "dev-plain",
			`[{"inline":[{"path":"/etc/file","content":"device-secret","mode":420}],"name":""}]`,
			`[{"appType":"container","name":"app","image":"img:v1","envVars":{"TOKEN":"app-secret"}}]`,
		).Error).To(Succeed())
		...
		Expect(db.WithContext(ctx).First(&device, "org_id = ? AND name = ?", orgId, "dev-plain").Error).To(Succeed())
		cfgJSON, err := json.Marshal(device.RenderedConfig)
		Expect(err).ToNot(HaveOccurred())
		Expect(string(cfgJSON)).ToNot(ContainSubstring("device-secret"))
		Expect(string(cfgJSON)).To(ContainSubstring("enc:v1:default:"))
		appsJSON, err := json.Marshal(device.RenderedApplications)
		Expect(err).ToNot(HaveOccurred())
		Expect(string(appsJSON)).ToNot(ContainSubstring("app-secret"))
		Expect(string(appsJSON)).To(ContainSubstring("enc:v1:default:"))
	})
	...
	It("When the active key target changes it should reset a completed checkpoint", func() {
		...
		Expect(err).ToNot(HaveOccurred())
		Expect(cp.Set(ctx, tasks.EncryptionMigrationConsumer, tasks.EncryptionMigrationCheckpointKey(domain.RepositoryKind, orgId), stale)).To(Succeed())
	...
	It("When two workers contend for an org lease only one acquires it", func() {
		...
		// Two locker instances; each TryLock pins its own session-scoped Conn.
		locker1 := tasks.NewPostgresEncryptionMigrationLocker(db)
		locker2 := tasks.NewPostgresEncryptionMigrationLocker(db)
		key := domain.RepositoryKind + ":" + orgId.String()
		unlock1, ok1, err := locker1.TryLock(ctx, key)
		Expect(err).ToNot(HaveOccurred())
		Expect(ok1).To(BeTrue())

		_, ok2, err := locker2.TryLock(ctx, key)
		Expect(err).ToNot(HaveOccurred())
		Expect(ok2).To(BeFalse())

		Expect(unlock1()).To(Succeed())

		unlock3, ok3, err := locker2.TryLock(ctx, key)
		Expect(err).ToNot(HaveOccurred())
