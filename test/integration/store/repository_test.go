...
var _ = Describe("RepositoryStore create", func() {
	...
	Context("Repository store", func() {
		...
		It("Create OCI repository with credentials", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(ociSpec.Url).To(Equal("quay.io/myorg"))
			Expect(ociSpec.Type).To(Equal(api.Oci))
			Expect(*ociSpec.AccessMode).To(Equal(api.ReadWrite))
			Expect(*ociSpec.Username).To(Equal("myuser"))
			Expect(*ociSpec.Password).To(Equal("mypassword"))
		})
		...
		It("Create OCI repository without credentials (public registry)", func() {
			...
			ociSpec, err := repo.Spec.GetOciRepoSpec()
			Expect(err).ToNot(HaveOccurred())
			Expect(ociSpec.Url).To(Equal("registry.redhat.io"))
			Expect(*ociSpec.AccessMode).To(Equal(api.Read))
			Expect(ociSpec.Username).To(BeNil())
			Expect(ociSpec.Password).To(BeNil())
		})
		...
		It("List OCI repositories by accessMode using FieldSelector", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(ociSpec.Url).To(Equal("quay.io/output"))
			Expect(ociSpec.Type).To(Equal(api.Oci))
			Expect(*ociSpec.AccessMode).To(Equal(api.ReadWrite))
		...
		It("Get OCI repository and verify accessMode field", func() {
			...
			ociSpec, err := repo.Spec.GetOciRepoSpec()
			Expect(err).ToNot(HaveOccurred())
			Expect(ociSpec.Url).To(Equal("quay.io/myproject"))
			Expect(ociSpec.Type).To(Equal(api.Oci))
			Expect(*ociSpec.AccessMode).To(Equal(api.ReadWrite))
			Expect(*ociSpec.Username).To(Equal("testuser"))
			Expect(*ociSpec.Password).To(Equal("testpass"))
		})
