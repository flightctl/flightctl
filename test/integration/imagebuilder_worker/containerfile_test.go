...
var _ = Describe("Containerfile Generation", func() {
	...
	BeforeEach(func() {
		...
		erHandler := enrollmentrequestservice.NewServiceHandler(enrollmentRequestStore, nil, nil, caClient, nil, eventsSvc, log, nil, "", "", nil, nil)
	...
	Context("Late binding containerfile generation", func() {
		It("should generate containerfile with correct content for late binding", func() {
			...
			result, err := tasks.GenerateContainerfile(ctx, repositorySvc, serviceHandler, orgId, loadedBuild, log)
	...
	Context("Error handling", func() {
		It("should return error when repository not found", func() {
			...
			_, err = tasks.GenerateContainerfile(ctx, repositorySvc, serviceHandler, orgId, loadedBuild, log)
