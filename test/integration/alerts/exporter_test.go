...
var _ = Describe("Alert Exporter", func() {
	...
	Context("Error Handling Scenarios", func() {
		...
		It("handles events with missing required fields", func() {
			...
			// Create valid event
			createEvent(ctx, serviceHandler, api.DeviceDiskWarning, api.DeviceKind, "valid-device")
		...
		It("handles high volume of events without timeout", func() {
			...
			for i := 0; i < numDevices; i++ {
				...
				createEvent(ctx, serviceHandler, api.DeviceCPUWarning, api.DeviceKind, deviceName)
			}
	...
	Context("Alert Deduplication and Edge Cases", func() {
		...
		It("handles rapid alert state transitions", func() {
			...
			createEvent(ctx, serviceHandler, api.DeviceCPUWarning, api.DeviceKind, deviceName)
			createEvent(ctx, serviceHandler, api.DeviceCPUCritical, api.DeviceKind, deviceName)
			createEvent(ctx, serviceHandler, api.DeviceCPUNormal, api.DeviceKind, deviceName)
			createEvent(ctx, serviceHandler, api.DeviceCPUWarning, api.DeviceKind, deviceName)
		...
		It("processes non-alertable events without creating alerts", func() {
			...
			createEvent(ctx, serviceHandler, api.ResourceCreated, api.FleetKind, fmt.Sprintf("%s-fleet1", prefix))
			createEvent(ctx, serviceHandler, api.ResourceUpdated, api.DeviceKind, fmt.Sprintf("%s-dev1", prefix))
			createEvent(ctx, serviceHandler, api.DeviceContentOutOfDate, api.DeviceKind, fmt.Sprintf("%s-dev2", prefix))
			createEvent(ctx, serviceHandler, api.DeviceContentUpdating, api.DeviceKind, fmt.Sprintf("%s-dev3", prefix))

			// Add one alertable event to ensure processing works
			createEvent(ctx, serviceHandler, api.DeviceCPUWarning, api.DeviceKind, fmt.Sprintf("%s-dev4", prefix))
