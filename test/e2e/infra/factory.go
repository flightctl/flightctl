...
// Create once at harness/suite creation via infra/setup.NewProvidersForEnvironment or setup.EnsureDefaultProviders;
// after that use the interface methods (e.g. GetSecretsProvider().GetSecretData) without
// further environment checks. Methods that are not supported in an implementation
// (e.g. GetSecretData on Quadlet) return an error (e.g. ErrSecretsNotSupported);
// callers handle that instead of branching on implementation type.
...
func GetEnvironmentConfig() *EnvironmentConfig {
	...
	if namespace == "" {
		namespace = os.Getenv("FLIGHTCTL_NS") // scripts (e.g. run_e2e_tests.sh) often set this
	}
