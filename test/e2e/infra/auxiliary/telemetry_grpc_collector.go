...
func (c *TelemetryGRPCCollector) MetricsContain(ctx context.Context, value string) (bool, error) {
	...
	if c.proxy != nil && !c.proxy.IsRunning() {
		return false, fmt.Errorf("telemetry gRPC collector proxy is not running")
	}
	select {
	case <-ctx.Done():
...
// startTelemetryGRPCProxy exposes the host receiver through a host-published
// container port so the test deployment can reach it through E2E_AUX_HOST.
...
// metricsRequestContains checks metric names and serialized request attributes
// for the expected device identifier.
...
// telemetryGRPCServerCertificate creates the short-lived certificate used by
// the test-only gRPC receiver.
