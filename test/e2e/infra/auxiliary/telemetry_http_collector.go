...
const (
	...
	telemetryHTTPCollectorKindNetwork    = "kind"
...
func (c *TelemetryHTTPCollector) Start(ctx context.Context, network string, reuse bool) error {
	...
	if network == telemetryHTTPCollectorKindNetwork {
		// Kind workloads run inside the kind network. Use the collector's
		// network address instead of crossing the host's published-port path.
		c.Host, err = container.ContainerIP(ctx)
		if err != nil {
			return fmt.Errorf("get telemetry HTTP collector container IP: %w", err)
		}
		if strings.TrimSpace(c.Host) == "" {
			return fmt.Errorf("telemetry HTTP collector container IP is empty")
		}
		c.Port = telemetryHTTPCollectorPortNum
	} else {
		c.Host = GetHostIP()
