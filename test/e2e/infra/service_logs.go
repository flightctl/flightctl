...
func ServiceFailureObserver(ctx context.Context, lifecycle ServiceLifecycleProvider, logs ServiceLogProvider, service ServiceName, timeout time.Duration, tailLines int, expectedLog string) (string, error) {
	if lifecycle == nil || logs == nil || timeout <= 0 || tailLines <= 0 || strings.TrimSpace(expectedLog) == "" {
		return "", fmt.Errorf("service failure observer requires providers, positive timeout and tail lines, and an expected log message")
	}
	if ctx == nil {
		return "", fmt.Errorf("service failure observer context is nil")
	}
