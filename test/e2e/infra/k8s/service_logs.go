...
func (p *InfraProvider) GetServiceLogs(ctx context.Context, service infra.ServiceName, tailLines int) (string, error) {
	if p == nil {
		return "", fmt.Errorf("Kubernetes infra provider is nil")
	}
	if p.client == nil {
		return "", fmt.Errorf("no Kubernetes client")
	}
	if ctx == nil {
		return "", fmt.Errorf("Kubernetes service log context is nil")
	}
	if tailLines <= 0 {
...
// podLogs reads bounded current or previous logs for one Kubernetes pod.
func (p *InfraProvider) podLogs(ctx context.Context, namespace, podName string, tailLines int, previous bool) (logs string, err error) {
	...
	if ctx == nil {
		return "", fmt.Errorf("pod log context is nil")
	}
	if strings.TrimSpace(namespace) == "" || strings.TrimSpace(podName) == "" {
		return "", fmt.Errorf("pod namespace and name are required")
	}
	if tailLines <= 0 {
		return "", fmt.Errorf("pod log tail lines must be positive")
	}
