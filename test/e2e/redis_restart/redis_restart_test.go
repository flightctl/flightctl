...
var _ = Describe("Redis Restart Tests", func() {
	...
	// It also sets up defer cleanup for each repository
	...
	// It also sets up defer cleanup for each fleet
	createTestFleets := func(harness *e2e.Harness, count int, prefix, testID string) []string {
		...
		for i := range fleetNames {
			...
			Expect(err).ToNot(HaveOccurred())
			defer func(name string) { _, _ = resources.Delete(harness, util.Fleet, name) }(fleetNames[i])
		}
		return fleetNames
	}

	// createTestFleet creates a single test fleet and returns its name
	// It also sets up defer cleanup
	...
	BeforeEach(func() {
		...
		// Get deployment context from harness
		context = harness.GetContext()
		GinkgoWriter.Printf("Detected deployment context: %s\n", context)
	})
