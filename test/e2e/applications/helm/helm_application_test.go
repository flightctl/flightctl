...
var _ = Describe("VM Agent Helm Application Tests", Ordered, func() {
	...
	BeforeEach(func() {
		...
		registryEndpoint := harness.RegistryEndpoint()
		privateEndpoint := harness.PrivateRegistryEndpoint()
		// helm doesn't support the same redirection that podman does
	...
	Context("helm application", func() {
		It("should deploy, update, and remove a helm application on microshift", Label("OCP-87529", "agent", "slow"), func() {
			...
			harness.WaitForApplicationStatusByName(deviceId, helmAppName, v1beta1.ApplicationStatusRunning)
			...
			harness.WaitForApplicationsSummaryStatus(deviceId, v1beta1.ApplicationsSummaryStatusNoApplications)
		...
		It("should deploy multiple application types and manage their lifecycle", Label("OCP-87530", "sanity", "agent", "slow"), func() {
			...
			harness.WaitForApplicationStatusByName(deviceId, quadletAppName, v1beta1.ApplicationStatusRunning)
			harness.WaitForApplicationStatusByName(deviceId, containerAppName, v1beta1.ApplicationStatusRunning)
			harness.WaitForApplicationStatusByName(deviceId, helmAppName, v1beta1.ApplicationStatusRunning)
			harness.WaitForApplicationsSummaryStatus(deviceId, v1beta1.ApplicationsSummaryStatusHealthy)
			...
			rootlessContainerAppSpec, err := e2e.NewContainerApplicationSpec(containerAppName, nginxImage, flightctlUser, []v1beta1.ApplicationPort{}, nil, nil, nil)
			Expect(err).ToNot(HaveOccurred())
		...
		It("should report degraded application status when pods are unavailable", Label("OCP-87531", "sanity", "agent", "slow"), func() {
			...
			harness.WaitForApplicationsSummaryStatus(deviceId, v1beta1.ApplicationsSummaryStatusError)
		...
		It("should validate helm application error handling across multiple failure scenarios", Label("OCP-87532", "agent", "slow"), func() {
			...
			username, password, err := harness.GetRegistryAuth()
			Expect(err).ToNot(HaveOccurred())
			authJSON := harness.BuildAuthJSON(username, password, authFlightctlRepo)
		...
		It("runAs flightctl applications can be deployed in a device", Label("OCP-87874", "agent", "slow"), func() {
			...
			containerAppSpec, err := e2e.NewContainerApplicationSpec(containerAppName, nginxImage, flightctlUser, []v1beta1.ApplicationPort{}, nil, nil, nil)
			Expect(err).ToNot(HaveOccurred())
		...
		It("runAs flightctl application with auth can be deployed to a device", Label("OCP-88004", "sanity", "agent", "slow"), func() {
			By("Deploy a helm app with helm registry credentials")
			user, pass, err := harness.GetRegistryAuth()
			Expect(err).ToNot(HaveOccurred())
			creds := harness.BuildAuthJSON(user, pass, harness.PrivateRegistryEndpoint(), authFlightctlRepo)
		...
		It("Helm application images can be pruned", Label("OCP-88005", "agent", "slow"), func() {
