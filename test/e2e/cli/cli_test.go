...
var _ = Describe("cli operation", func() {
	...
	Context("Flightctl Version Checks", func() {
		...
		It("should show FIPS runtime compliance", Label("rpm-sanity", "sanity", "84648"), func() {
			harness := e2e.GetWorkerHarness()
			...
			cmd := exec.Command("bash", "-c", "GOLANG_FIPS=1 OPENSSL_FORCE_FIPS_MODE=1 LD_DEBUG=symbols flightctl version 2>&1 | grep OPENSSL")
			output, err := cmd.CombinedOutput()
			Expect(err).ToNot(HaveOccurred(), "Failed to run flightctl with FIPS environment")

			// Check that we got some OpenSSL symbol loading output
			openSSLOutput := string(output)
