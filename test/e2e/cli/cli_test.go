...
var _ = Describe("cli login", func() {
	...
	It("generates completion and can be sourced for each supported shell (harness.CLI only for flightctl calls)", Label("85470"), func() {
		...
		By("Autocompletion: Scenario A (get <resource>/<prefix>)")
		ExpectCompletion(harness,
			[]string{"get", util.Device, devName[:1]},
			devName)
		ExpectCompletion(harness,
			[]string{"get", util.Fleet, fleetName[:1]},
			fleetName)
		ExpectCompletion(harness,
			[]string{"get", util.CertificateSigningRequest, csrName[:1]},
			csrName)
		ExpectCompletion(harness,
			[]string{"get", util.Repository, repoName[:1]},
			repoName)
		ExpectCompletion(harness,
			[]string{"get", util.EnrollmentRequest, erName[:1]},
			erName)

		By("Autocompletion: Scenario B (edit <resource>/<prefix>)")
		ExpectCompletion(harness,
			[]string{"edit", util.Device, devName[:1]},
			devName)
		ExpectCompletion(harness,
			[]string{"edit", util.Fleet, fleetName[:1]},
			fleetName)
		ExpectCompletion(harness,
			[]string{"edit", util.CertificateSigningRequest, csrName[:1]},
			csrName)
		ExpectCompletion(harness,
			[]string{"edit", util.Repository, repoName[:1]},
			repoName)
		ExpectCompletion(harness,
			[]string{"edit", util.EnrollmentRequest, erName[:1]},
			erName)

		By("Autocompletion: Scenario C (approve csr/er <prefix>)")
		ExpectCompletion(harness,
			[]string{"approve", util.CertificateSigningRequest, csrName[:1]},
			csrName)
		ExpectCompletion(harness,
			[]string{"approve", util.EnrollmentRequest, erName[:1]},
			erName)

		By("Autocompletion: Scenario D (deny csr/er <prefix>)")
		ExpectCompletion(harness,
			[]string{"deny", util.CertificateSigningRequest, csrName[:1]},
			csrName)
		ExpectCompletion(harness,
			[]string{"deny", util.EnrollmentRequest, erName[:1]},
			erName)

		By("Autocompletion: Scenario E (decommission device <prefix>)")
		ExpectCompletion(harness,
			[]string{"decommission", util.Device, devName[:1]},
			devName)
	})
