...
var _ = Describe("cli events operation", func() {
	...
	Context("Events API Tests", func() {
		...
		It("should show events for application workload validation", Label("83588", "sanity"), func() {
			...
			Eventually(func() string {
				out, err := harness.RunGetEvents(fieldSelector, fmt.Sprintf("involvedObject.name=%s,type=Warning,reason=%s", deviceName, util.DeviceApplicationError))
				if err != nil {
					return ""
				}
				return out
			}, "60s", "2s").Should(ContainSubstring(fmt.Sprintf("%s is in status Error", appName1)))
