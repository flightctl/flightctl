...
func collectorLogsContain(ctx context.Context, collector *auxiliary.TelemetryHTTPCollector, serviceLogs infra.ServiceLogProvider, substrings ...string) func() bool {
	reported := false
	return func() bool {
		...
		if err != nil {
			if !reported {
				GinkgoWriter.Printf("collector logs unavailable while waiting for %q: %v\n", strings.Join(substrings, ", "), err)
				reported = true
			}
			return false
		}
		if !found && !reported {
			GinkgoWriter.Printf("collector has not received expected telemetry; expected markers: %q; current log tail:\n%s\n", strings.Join(substrings, ", "), logs)
			logTelemetryGatewayForwardingDiagnostics(ctx, serviceLogs)
			reported = true
		}
		return found
	}
...
// logTelemetryGatewayForwardingDiagnostics prints bounded gateway logs without
// allowing credential-bearing header values into CI output.
func logTelemetryGatewayForwardingDiagnostics(ctx context.Context, serviceLogs infra.ServiceLogProvider) {
	if serviceLogs == nil {
		GinkgoWriter.Println("telemetry gateway logs unavailable: service log provider is nil")
		return
	}
	ioCtx, cancel := context.WithTimeout(ctx, FIVESECTIMEOUT)
	defer cancel()
	...
	if err != nil {
		GinkgoWriter.Printf("telemetry gateway logs unavailable: %v\n", err)
		return
	}
	GinkgoWriter.Printf("telemetry gateway logs (sensitive values redacted):\n%s\n", redactTelemetryGatewayLogs(logs))
}

func redactTelemetryGatewayLogs(logs string) string {
	lines := strings.Split(logs, "\n")
	for i, line := range lines {
		lower := strings.ToLower(line)
		if strings.Contains(lower, "authorization") || strings.Contains(lower, "api-token") || strings.Contains(lower, "x-custom-header") {
			lines[i] = "[redacted telemetry gateway log line]"
		}
	}
...
func grpcCollectorContains(ctx context.Context, collector *auxiliary.TelemetryGRPCCollector, value string) func() bool {
	...
	return func() bool {
		...
		if !found && !reported {
			count, countErr := collector.RequestCount()
			if countErr != nil {
				GinkgoWriter.Printf("gRPC collector request count unavailable: %v\n", countErr)
			} else {
				GinkgoWriter.Printf("gRPC collector has not received expected metric; retained requests: %d\n", count)
			}
			reported = true
		}
		return found
