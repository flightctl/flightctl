...
const (
	...
	telemetryForwardGitHubActionsOverride = "FLIGHTCTL_RUN_TELEMETRY_FORWARD_E2E"
	telemetryForwardDiagnosticLogBytes    = 4096
	telemetryForwardDiagnosticLogLines    = 20
...
var _ = Describe("Telemetry gateway forwarding", Serial, Label("observability"), func() {
	...
	It("should forward Flight Control device telemetry over OTLP/HTTP with custom headers", Label("90533", "sanity", "agent"), func(ctx SpecContext) {
		...
		By("restarting the device OTEL collector after gateway forwarding is configured")
		Expect(restartOTelCollector(ctx, harness)).To(Succeed())

		By("verifying the HTTP collector received the device telemetry and headers")
		err = waitForTelemetryHTTPForwarding(
			ctx,
			collector,
			TIMEOUT,
			POLLING,
	...
	It("should fail startup when a forward header references an undefined environment variable", Label("90535", "sanity"), func(ctx SpecContext) {
		...
		logs, err := infra.ServiceFailureObserver(ctx, providers.Lifecycle, serviceLogs, infra.ServiceTelemetryGateway, TENSECTIMEOUT, telemetryGatewayLogTailLines, telemetryMissingEnvError)
		Expect(err).ToNot(HaveOccurred())
	...
	It("should accept a bare host:port forward endpoint for gRPC backward compatibility", Label("90536", "sanity"), func(ctx SpecContext) {
		...
		By("verifying the gateway accepted the bare endpoint")
		// WaitForReady above is the positive assertion: a bare endpoint must select
		// gRPC configuration without rejecting telemetry gateway startup.
	})
	...
	It("should reject HTTP forward endpoints with query strings", Label("90537", "sanity"), func(ctx SpecContext) {
		...
		logs, err := infra.ServiceFailureObserver(ctx, providers.Lifecycle, serviceLogs, infra.ServiceTelemetryGateway, TENSECTIMEOUT, telemetryGatewayLogTailLines, telemetryQueryStringError)
		Expect(err).ToNot(HaveOccurred())
	...
	It("should reject custom forward headers on bare gRPC endpoints", Label("90538", "sanity"), func(ctx SpecContext) {
		...
		logs, err := infra.ServiceFailureObserver(ctx, providers.Lifecycle, serviceLogs, infra.ServiceTelemetryGateway, TENSECTIMEOUT, telemetryGatewayLogTailLines, telemetryGRPCHeadersError)
		Expect(err).ToNot(HaveOccurred())
...
func collectorLogs(ctx context.Context, collector *auxiliary.TelemetryHTTPCollector) (string, error) {
	if ctx == nil {
		return "", fmt.Errorf("collector log context is nil")
	}
	if collector == nil {
...
// waitForTelemetryHTTPForwarding waits for the expected collector markers and
// returns concise collector diagnostics when forwarding does not arrive.
func waitForTelemetryHTTPForwarding(ctx context.Context, collector *auxiliary.TelemetryHTTPCollector, timeout, polling time.Duration, markers ...string) error {
	if ctx == nil {
		return fmt.Errorf("telemetry HTTP forwarding context is nil")
	}
	if collector == nil {
		return fmt.Errorf("telemetry HTTP forwarding collector is nil")
	}
	if timeout <= 0 || polling <= 0 {
		return fmt.Errorf("telemetry HTTP forwarding timeout and polling must be positive")
	}
	if len(markers) == 0 {
		return fmt.Errorf("telemetry HTTP forwarding markers are empty")
	}
	for _, marker := range markers {
		if strings.TrimSpace(marker) == "" {
			return fmt.Errorf("telemetry HTTP forwarding marker is empty")
		}
	}
	...
	var lastErr error
	for {
		logs, err := collectorLogs(waitCtx, collector)
		if err != nil {
			lastErr = err
		} else {
			lastLogs = logs
			if containsAllMarkers(logs, markers...) {
				return nil
			}
		}
		select {
		case <-waitCtx.Done():
			return telemetryHTTPForwardingError(collector, markers, lastLogs, lastErr)
		case <-ticker.C:
		}
	}
}

// containsAllMarkers reports whether every expected marker occurs in logs.
func containsAllMarkers(logs string, markers ...string) bool {
	if logs == "" || len(markers) == 0 {
		return false
	}
	for _, marker := range markers {
...
// telemetryHTTPForwardingError builds a bounded, secret-safe forwarding failure.
func telemetryHTTPForwardingError(collector *auxiliary.TelemetryHTTPCollector, markers []string, logs string, logsErr error) error {
	endpoint := "<unknown>"
	if collector != nil && strings.TrimSpace(collector.Endpoint) != "" {
		endpoint = collector.Endpoint
	}
	diagnostic := fmt.Sprintf("telemetry HTTP forwarding timed out; endpoint=%s; expected markers=%q; collector log tail:\n%s", endpoint, markers, summarizeTelemetryLogs(logs))
	if logsErr != nil {
		diagnostic += fmt.Sprintf("; last log read error: %v", logsErr)
	}
	return fmt.Errorf("%s", diagnostic)
}

// summarizeTelemetryLogs keeps forwarding failure output bounded and secret-safe.
func summarizeTelemetryLogs(logs string) string {
	if strings.TrimSpace(logs) == "" {
		return "<empty>"
	}
	lines := strings.Split(logs, "\n")
	if len(lines) > telemetryForwardDiagnosticLogLines {
		lines = lines[len(lines)-telemetryForwardDiagnosticLogLines:]
	}
	summary := strings.Join(lines, "\n")
	if len(summary) > telemetryForwardDiagnosticLogBytes {
		summary = summary[len(summary)-telemetryForwardDiagnosticLogBytes:]
	}
	return summary
}

// skipGitHubActionsTelemetryForwarding skips device-to-collector forwarding
// specs in GitHub Actions, where the required host networking is unavailable.
// Set FLIGHTCTL_RUN_TELEMETRY_FORWARD_E2E=true to run them explicitly.
func skipGitHubActionsTelemetryForwarding() {
	if os.Getenv("GITHUB_ACTIONS") == "true" && os.Getenv(telemetryForwardGitHubActionsOverride) != "true" {
		Skip("telemetry forwarding E2E requires the host networking available in Jenkins")
	}
}

// grpcCollectorContains polls the bounded OTLP/gRPC receiver history for a value.
func grpcCollectorContains(ctx context.Context, collector *auxiliary.TelemetryGRPCCollector, value string) func() (bool, error) {
	if ctx == nil || collector == nil || strings.TrimSpace(value) == "" {
		return func() (bool, error) {
			return false, fmt.Errorf("gRPC collector polling requires a context, collector and value")
		}
	}
	return func() (bool, error) {
		ioCtx, cancel := context.WithTimeout(ctx, FIVESECTIMEOUT)
		defer cancel()
		return collector.MetricsContain(ioCtx, value)
	}
}
...
func cleanupTelemetryGatewayEnv(providers *infra.Providers, originalConfig, envName string) error {
	if strings.TrimSpace(envName) == "" {
		return fmt.Errorf("telemetry gateway environment variable name is empty")
	}
...
func restoreTelemetryGatewayConfig(providers *infra.Providers, originalConfig string) error {
	...
	if strings.TrimSpace(originalConfig) == "" {
...
func telemetryGatewayConfigWithForward(original string, forward telemetryGatewayForwardConfig) (string, error) {
	...
	if err := replaceMappingValue(tg, "forward", forwardNode); err != nil {
		return "", err
	}
...
func validateTelemetryGatewayForwardNode(tg *yaml.Node) error {
	if tg == nil || tg.Kind != yaml.MappingNode {
		return fmt.Errorf("telemetry gateway section is not a mapping")
	}
...
// validateTelemetryGatewayForwardConfig validates the replacement forward
// configuration before it is serialized into the live service config.
...
// telemetryGatewayRootNode returns the mapping node at the YAML document root.
...
// telemetryGatewaySectionNode returns the telemetry gateway section mapping.
func telemetryGatewaySectionNode(root *yaml.Node) (*yaml.Node, error) {
	if root == nil || root.Kind != yaml.MappingNode {
		return nil, fmt.Errorf("telemetry gateway config root is not a mapping")
	}
...
// telemetryGatewaySectionCount counts accepted telemetry gateway section keys.
...
// yamlNodeForForwardConfig converts the replacement forward config to a YAML node.
...
// replaceMappingValue replaces or appends a value in a YAML mapping.
func replaceMappingValue(mapping *yaml.Node, key string, value *yaml.Node) error {
	if mapping == nil || mapping.Kind != yaml.MappingNode {
		return fmt.Errorf("YAML mapping is nil or has an unexpected kind")
	}
	if strings.TrimSpace(key) == "" || value == nil {
		return fmt.Errorf("YAML mapping key and value are required")
	}
	for i := 0; i+1 < len(mapping.Content); i += 2 {
		if mapping.Content[i].Value == key {
