	...
	Describe("Test Setup", Ordered, func() {
		...
		Context("microshift", func() {
			It(`Verifies that a microshift cluster can enroll to acm`, Label("75967"), func() {
				...
				stdout, err = harness.VM.RunSSH([]string{"stat", psPath}, nil)
				Expect(err).ToNot(HaveOccurred())
				...
				echo %s
				`, kubeconfigPath, readyMsg)
				stdout, err = harness.VM.RunSSH([]string{"sudo", "bash", "-c", script}, nil)
				Expect(err).ToNot(HaveOccurred())
				...
				testFleetDeviceSpec := createAcmRegistrationFleetDeviceSpec(harness, inlineConfigProviderSpec)
				...
				_, err = harness.VM.RunSSH([]string{"sudo", "oc", "wait", fmt.Sprintf("--for=condition=Ready pods --all -A --timeout=300s --kubeconfig=%s", kubeconfigPath)}, nil)
				Expect(err).ToNot(HaveOccurred())

				By("Check that the cluster is registered in acm")
				err = waitForClusterRegistered(deviceId, harness, 5*time.Minute)
				Expect(err).ToNot(HaveOccurred())
			})
		})
	})
...
var testFleetDeviceSpec v1alpha1.DeviceSpec
...
func createAcmRegistrationFleetDeviceSpec(harness *e2e.Harness, pullSecretinlineConfigProviderSpec v1alpha1.ConfigProviderSpec) v1alpha1.DeviceSpec {
	hookFile := fmt.Sprintf("%s/test/e2e/microshift_acm_enrollment/data/acm-hook.yaml", util.GetTopLevelDir())
	inlineContentSeconByte, err := os.ReadFile(hookFile)
	Expect(err).ToNot(HaveOccurred())
	inlineContentSecond := string(inlineContentSeconByte)

	var inlineConfigSecondSpec = v1alpha1.FileSpec{
	...
	configProviderSpec := []v1alpha1.ConfigProviderSpec{pullSecretinlineConfigProviderSpec, inlineConfigProviderSpecSecond, httpConfigProviderSpec, httpConfigProviderSpecSecond}

	logrus.Infof("this is the configProviderSpec %v", configProviderSpec)
	...
	testFleetDeviceSpec.Os = &osImageSpec
	testFleetDeviceSpec.Config = &configProviderSpec

	return testFleetDeviceSpec
}
...
func getAcmNamespace() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to get acm namespace")
	}
...
func waitForMicroshiftReady(harness *e2e.Harness, kubeconfigPath string) error {
	...
	for time.Since(start) < timeout {
		cmd := []string{
			"sudo", "oc", "wait",
			fmt.Sprintf("--for=condition=Ready pods --all -A --timeout=30s --kubeconfig=%s", kubeconfigPath),
		}
...
func waitForClusterRegistered(deviceId string, harness *e2e.Harness, timeout time.Duration) error {
	start := time.Now()
