...
func printVMQuadletDiagnosticsIfFailed(h *e2e.Harness) {
	...
	cmds := []struct {
		...
		{"podman ps", []string{"sudo", "podman", "ps", "--format", "{{.ID}} {{.Names}} {{.Status}} {{.Ports}}"}},
		{"podman ps all", []string{"sudo", "podman", "ps", "-a", "--format", "{{.ID}} {{.Names}} {{.Status}} {{.Ports}}"}},
		{"listeners 2222", []string{"sudo", "ss", "-ltnp", "sport = :2222"}},
