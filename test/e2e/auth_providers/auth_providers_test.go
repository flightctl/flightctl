...
const (
	...
	authRequiredMsg     = "login provider suite requires authentication to be enabled"
	getDevicesCmd       = "devices"
	tokenDuration       = "1h"
	skipTLSVerifyFlag   = "--insecure-skip-tls-verify"
	pamAdminGroup       = "flightctl-admin"
	adminServiceAccount = "flightctl-admin"
	denyServiceAccount  = "deny-user"
...
var deniedAccessErrorSubstrings = []string{
	forbiddenStatusOutput,
	"forbidden",
	"permission",
	"denied",
}
...
var _ = Describe("Login providers", Ordered, Label("login-providers"), func() {
	...
	BeforeEach(func() {
		...
		Expect(harness.IsAuthDisabled(apiEndpoint, checkCfgDir)).To(BeFalse(), authRequiredMsg)
	})

	It("runs PAM positive and negative auth flow", Label("88164", "sanity", "Agent"), func() {
		if !caps.IsKind {
			Skip("PAM issuer flow is currently validated only on KIND context")
		}
		...
		pamAdminUser, pamNonAdminUser := buildPAMTestUsers(harness.GetTestIDFromContext())
		DeferCleanup(harness.DeletePAMUser, pamIssuerContainerName(), pamNonAdminUser)
		DeferCleanup(harness.DeletePAMUser, pamIssuerContainerName(), pamAdminUser)

		err = harness.ProvisionPAMUser(pamIssuerContainerName(), pamAdminUser, pamAdminPass, []string{pamAdminGroup})
		Expect(err).ToNot(HaveOccurred())
		...
		bad := runFlightctlCmd(harness, cfgDir, "", "login", apiEndpoint, "--provider", providerName, "-u", pamAdminUser, "-p", pamAdminPass+"-bad", skipTLSVerifyFlag)
		...
		nonAdminLogin := runFlightctlCmd(harness, cfgDir, "", "login", apiEndpoint, "--provider", providerName, "-u", pamNonAdminUser, "-p", pamNonAdminPass, skipTLSVerifyFlag)
		...
		denied := runFlightctlCmd(harness, cfgDir, "", "get", getDevicesCmd)
		...
		Expect(util.ContainsAnySubstring(strings.ToLower(denied.Combined), deniedAccessErrorSubstrings)).To(BeTrue(), "non-admin output missing denied/403 markers")
	...
	It("runs Kubernetes positive and negative token flow", Label("88166", "sanity", "Agent"), func() {
		if !caps.IsOpenShift {
			Skip("Kubernetes token flow is currently validated only when OCP context is enabled")
		}
		if !caps.HasKubectl {
		...
		tokenOut, err := util.CreateK8UserToken(ns, adminServiceAccount, tokenDuration)
		Expect(err).ToNot(HaveOccurred())
		...
		bad := runFlightctlCmd(harness, cfgDir, "", "login", apiEndpoint, "--token", token+"broken", skipTLSVerifyFlag)
		...
		err = harness.EnsureNamespace(denyNs)
		Expect(err).ToNot(HaveOccurred())
		defer harness.CleanupNamespace(denyNs)
		err = harness.EnsureServiceAccount(denyNs, denyServiceAccount)
		Expect(err).ToNot(HaveOccurred())
		denyTokenOut, err := util.CreateK8UserToken(denyNs, denyServiceAccount, tokenDuration)
		Expect(err).ToNot(HaveOccurred())
		denyToken := strings.TrimSpace(denyTokenOut)
		Expect(denyToken).ToNot(BeEmpty())
		loginRes := runFlightctlCmd(harness, cfgDir, "", "login", apiEndpoint, "--token", denyToken, skipTLSVerifyFlag)
		expectZeroWithTrace(providerK8s, denyServiceAccount+" login", loginRes, caps, map[string]string{"namespace": denyNs})
		denied := runFlightctlCmd(harness, cfgDir, "", "get", getDevicesCmd)
		...
		Expect(util.ContainsAnySubstring(strings.ToLower(denied.Combined), deniedAccessErrorSubstrings)).To(BeTrue(), "denied output missing denied/403 markers")
	...
	It("runs AAP positive and negative flow when credentials are configured", Label("88169", "sanity", "Agent"), func() {
		if !caps.IsQuadlets {
			Skip("AAP flow is currently validated only in quadlets environments")
		}
...
func runAuthenticatedProviderFlow(h *e2e.Harness, cfgDir, apiEndpoint string, spec loginSpec, caps envCapabilities) error {
	By("login succeeds")
	login, err := loginAndExpectSuccess(h, cfgDir, apiEndpoint, spec, caps)
	if err != nil {
		return err
	}
...
func loginAndExpectSuccess(h *e2e.Harness, cfgDir, apiEndpoint string, spec loginSpec, caps envCapabilities) (cmdResult, error) {
	login := runFlightctlCmd(h, cfgDir, "", loginCommandArgs(apiEndpoint, spec)...)
	if err := validateSuccessfulLoginResult(spec.ProviderName, login, caps); err != nil {
		return cmdResult{}, err
	}
	return login, nil
}

func validateSuccessfulLoginResult(provider string, login cmdResult, caps envCapabilities) error {
	if login.ExitCode != 0 {
		return fmt.Errorf("%s", withTrace(provider, "positive login expected success", login, caps, map[string]string{"provider": provider}))
	}
	if !strings.Contains(login.Combined, loginSuccessfulOutput) {
		return fmt.Errorf("login output missing success marker: %s", withTrace(provider, "positive login", login, caps, nil))
	}
	return nil
}
...
func runInvalidCredsNegativeIfPossible(provider string, h *e2e.Harness, cfgDir, apiEndpoint string, spec loginSpec, caps envCapabilities) error {
	...
	if spec.UseToken {
		bad := runFlightctlCmd(h, cfgDir, "", "login", apiEndpoint, "--token", spec.Token+"broken", skipTLSVerifyFlag)
		if bad.ExitCode == 0 {
	...
	bad := runFlightctlCmd(h, cfgDir, "", "login", apiEndpoint, "--provider", spec.ProviderName, "-u", spec.Username, "-p", spec.Password+"-bad", skipTLSVerifyFlag)
	if bad.ExitCode == 0 {
...
func detectEnvCapabilities(h *e2e.Harness, apiEndpoint string) envCapabilities {
	detectedEnvironment := infra.DetectEnvironment()
	caps := envCapabilities{
		Context:           detectedEnvironment,
		IsOpenShift:       detectedEnvironment == infra.EnvironmentOCP,
		IsKind:            detectedEnvironment == infra.EnvironmentKind,
		...
		IsQuadlets:        detectedEnvironment == infra.EnvironmentQuadlet,
		AuthProviderInfo:  map[string]string{},
	}
	if caps.HasOc {
...
func loginCommandArgs(apiEndpoint string, spec loginSpec) []string {
	args := []string{"login", apiEndpoint, skipTLSVerifyFlag}
	if spec.UseToken {
...
func buildPAMTestUsers(testID string) (string, string) {
	suffix := "default"
