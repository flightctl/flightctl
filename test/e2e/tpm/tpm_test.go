	...
	Context("TPM Enrollment", func() {
		...
		It("Should enroll device with TPM enabled and verify attestation", Label("83974", "tpm", "sanity"), func() {
			...
			Eventually(func() bool {
				stdout, err := harness.VM.RunSSH([]string{"sudo", "journalctl", "-u", "flightctl-agent", "-n", "100", "--no-pager"}, nil)
				if err != nil {
					return false
				}
				logOutput := stdout.String()
				return strings.Contains(logOutput, "Using TPM-based identity provider")
			...
			Eventually(func() error {
				...
				if !hasAttestation {
					...
					tpmVendorInfo, hasTpmVendorInfo := systemInfo.Get("tpmVendorInfo")
					if hasTpmVendorInfo {
			...
			harness.ApproveEnrollment(enrollmentID, &v1alpha1.EnrollmentRequestApproval{
				Approved: true,
				Labels:   &labels,
			})
			...
			deviceId, _, err := harness.WaitForDeviceOnlineStatus(enrollmentID)
			Expect(err).ToNot(HaveOccurred())

			By("debugging device status structure")
			// Get fresh device status for debugging
			resp, err := harness.Client.GetDeviceWithResponse(harness.Context, deviceId)
			Expect(err).ToNot(HaveOccurred())
			Expect(resp.JSON200).ToNot(BeNil())
			device := resp.JSON200

			logrus.Infof("=== DEVICE STATUS DEBUG ===")
			logrus.Infof("Device ID: %s", deviceId)
			if device.Status != nil {
				logrus.Infof("Device Status exists")
				logrus.Infof("Device Status Integrity: %+v", device.Status.Integrity)
				if device.Status.Integrity.Tpm != nil {
					logrus.Infof("TPM Integrity Object: %+v", device.Status.Integrity.Tpm)
				}
				if device.Status.Integrity.DeviceIdentity != nil {
					logrus.Infof("Device Identity Integrity Object: %+v", device.Status.Integrity.DeviceIdentity)
				}
				logrus.Infof("SystemInfo keys available: %+v", device.Status.SystemInfo)
			} else {
				logrus.Infof("Device Status is nil")
			}
			logrus.Infof("=== END DEVICE STATUS DEBUG ===")
			...
			tpmVendorInfo, hasTpmVendorInfo := device.Status.SystemInfo.Get("tpmVendorInfo")
			if hasTpmVendorInfo {
				Expect(tpmVendorInfo).ToNot(BeEmpty())
				...
				Expect(deviceAttestation).ToNot(BeEmpty())
