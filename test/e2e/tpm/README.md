...
This directory contains End-to-End tests for **TPM (Trusted Platform Module) 2.0** device authentication and attestation functionality.

## Test Files

### 1. `tpm_virtual_test.go` - Virtual TPM (CI-Compatible) ✅
**Status:** Runs in CI

Tests TPM functionality using **virtual/software TPM** in VMs:
- ✅ Runs automatically in CI
- ✅ Uses VM harness like other e2e tests
- ✅ Validates TPM enrollment and attestation data
- ✅ Expects "Failed" integrity status (no certificate chain for virtual TPM)
- ✅ Verifies TPM-signed communication works

### 2. `tpm_test.go` - Real Hardware TPM (Manual Testing) 🔧
**Status:** Skipped in CI (labeled as "hardware")

Tests the **complete TPM verification workflow** on real hardware:
- 🔧 Requires manual execution on RHEL9 with real TPM
- 🔧 Installs agent from Copr repository
- 🔧 Validates certificate chain from device to manufacturer
- 🔧 Expects "Verified" integrity status
- 🔧 See [RUNNING_ON_RHEL9.md](RUNNING_ON_RHEL9.md) for instructions

## Overview

### Virtual TPM Test (tpm_virtual_test.go)

The virtual TPM test validates TPM functionality in CI using software TPM, including:

- ✅ TPM device detection (`/dev/tpm0`)
- ✅ TPM functionality verification (`tpm2_startup`)
- ✅ Agent TPM configuration
- ✅ TPM-based enrollment with attestation data
- ✅ Enrollment request verification
- ✅ Device approval and online status
- ✅ TPM integrity checks (expects "Failed" for virtual TPM)
- ✅ TPM key persistence (`tpm-blob.yaml`)
- ✅ TPM-signed device communication

### Real Hardware TPM Test (tpm_test.go)

The real hardware TPM test validates the complete verification workflow on real TPM, including:
...
### Virtual TPM Test (Automatic in CI)

```bash
# Run virtual TPM test (CI-compatible)
go test ./test/e2e/tpm -v -run "Virtual"

# Or run all tests
go test ./test/e2e/tpm -v
```

**No special prerequisites** - Uses standard e2e test infrastructure with VM harness.

### Real Hardware TPM Test (Manual Only)

See [RUNNING_ON_RHEL9.md](RUNNING_ON_RHEL9.md) for detailed instructions.

**Quick start:**
```bash
# On RHEL9 with real TPM hardware
sudo FLIGHTCTL_API_URL=https://api.flightctl.example.com \
     go test ./test/e2e/tpm -v -run "Real"
```

## Prerequisites

### For Virtual TPM Test (CI)
- ✅ Standard e2e test environment
- ✅ VM harness with virtual TPM
- ✅ No special configuration needed

### For Real Hardware TPM Test (Manual)
