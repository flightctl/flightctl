...
var _ = Describe("Dependency Sync", Label("dependency-sync"), func() {
	...
	Context("dependency sync", func() {
		It("should create a new TV, emit DependencyChangeDetected, and update device when a git commit is pushed", Label("89092", "sanity", "agent"), func() {
			...
			Eventually(dependencySyncProbeFailedPoll(harness, gitProbeRepoName), e2e.TIMEOUT, POLLING).Should(BeTrue(), probeCycleCompleteMsg)
			...
			Eventually(dependencyChangeDetectedPoll(harness, repoName, commitSHA), EVENTTIMEOUT, POLLING).Should(BeTrue(),
			...
			newVersion, err := harness.GetCurrentDeviceRenderedVersion(deviceID)
			Expect(err).ToNot(HaveOccurred())
			Expect(newVersion).To(BeNumerically(">", initialVersion))

			By("verifying the updated content was delivered to the device")
			content, err := harness.ReadFileFromDevice(deviceConfigFile)
			Expect(err).ToNot(HaveOccurred())
			Expect(content).To(ContainSubstring(updatedContentMarker))
		})
		...
		It("should create a new TV and show fingerprint in device status when HTTP content changes", Label("89091", "sanity", "agent"), func() {
			...
			Eventually(dependencySyncProbeFailedPoll(harness, httpProbeRepoName), e2e.TIMEOUT, POLLING).Should(BeTrue(), probeCycleCompleteMsg)
			...
			content, err := harness.ReadFileFromDevice(httpMountPath)
			Expect(err).ToNot(HaveOccurred())
			Expect(content).To(ContainSubstring(updatedHTTPContent))
		})
		...
		It("should sync the good fleet and emit DependencySyncProbeFailed for the bad fleet without blocking", Label("89093", "sanity"), func() {
			...
			Eventually(dependencySyncProbeFailedPoll(harness, badRepoName), e2e.TIMEOUT, POLLING).Should(BeTrue(),
			...
			badTVCount, err := harness.CountTemplateVersions(badFleetName)
			Expect(err).ToNot(HaveOccurred())
			Expect(badTVCount).To(BeNumerically("<=", 1),
			...
			Eventually(dependencySyncProbeFailedPoll(harness, badRepoName), EVENTTIMEOUT, POLLING).Should(BeTrue(),
		...
		It("should sync both git and HTTP providers independently in a multi-provider fleet", Label("89272", "sanity", "agent"), func() {
			...
			finalTVCount, err := harness.CountTemplateVersions(fleetName)
			Expect(err).ToNot(HaveOccurred())
			Expect(finalTVCount).To(BeNumerically(">", initialTVCount),
			...
			gitContent, err := harness.ReadFileFromDevice(deviceConfigFile)
			Expect(err).ToNot(HaveOccurred())
			Expect(gitContent).To(ContainSubstring(updatedContentMarker))

			httpContent, err := harness.ReadFileFromDevice(httpMountPath)
			Expect(err).ToNot(HaveOccurred())
			Expect(httpContent).To(ContainSubstring(updatedHTTPContent))
		})
		...
		It("should sync a parameterized targetRevision resolved per-device label", Label("89301", "sanity", "agent"), func() {
			...
			Eventually(dependencyChangeDetectedPoll(harness, repoName, commitSHA), e2e.TIMEOUT, POLLING).Should(BeTrue(),
			...
			finalTVCount, err := harness.CountTemplateVersions(fleetName)
			Expect(err).ToNot(HaveOccurred())
			Expect(finalTVCount).To(Equal(initialTVCount),
...
func dependencyChangeDetectedPoll(h *e2e.Harness, repoName, fingerprint string) func() (bool, error) {
	return func() (bool, error) {
		return h.HasEventWithDetails(v1beta1.EventReasonDependencyChangeDetected, repoName, fingerprint)
	}
}
...
func dependencySyncProbeFailedPoll(h *e2e.Harness, repoName string) func() (bool, error) {
	return func() (bool, error) {
		return h.HasEventWithDetails(v1beta1.EventReasonDependencySyncProbeFailed, repoName, "")
	}
}
