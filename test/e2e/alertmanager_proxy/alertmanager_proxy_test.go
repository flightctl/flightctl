...
var _ = Describe("Alertmanager proxy", func() {
	...
	It("lets a user discover and query alerts through the proxy", Label("87773", "sanity"), func() {
		...
		Expect(body).To(ContainSubstring(statusKindField), "malformed org_id response should include status kind")
		Expect(body).To(ContainSubstring(statusFailure), "malformed org_id response should include failure status")
