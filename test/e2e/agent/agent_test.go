		...
		It("Device status tests", Label("75991"), func() {
			...
			harness.WaitForDeviceContents(deviceId, fmt.Sprintf("Failed to update to renderedVersion: %s. Error", strconv.Itoa(newRenderedVersion)),
				func(device *v1alpha1.Device) bool {
					...
					return testutil.ConditionExists(device, "Updating", "False", string(v1alpha1.UpdateStateError)) ||
						(testutil.ConditionExists(device, "Updating", "False", string(v1alpha1.UpdateStateUpdated)) && (device.Status.Config.RenderedVersion == strconv.Itoa(previousRenderedVersion)))
				}, "2m")
			...
			harness.WaitForDeviceContents(deviceId, `Error: failed fetching specified Repository definition`,
				func(device *v1alpha1.Device) bool {
					return testutil.ConditionExists(device, "SpecValid", "False", "Invalid")
				}, "2m")
