...
var _ = Describe("VM Agent behavior during updates", func() {
	...
	Context("updates", func() {
		...
		It("reports systemd services and applications based on device spec", Label("sanity", "86238"), func() {
			...
			appProvider := v1beta1.ImageApplicationProviderSpec{Image: imageName}
			err = harness.UpdateApplication(true, deviceId, "my-compose-app", appProvider, nil)
			Expect(err).ToNot(HaveOccurred())
			...
			e2e.CreateFailingServiceOnDevice(harness, failingServiceName)
			...
			e2e.StopServiceOnDevice(harness, untrackedService)
			...
			Consistently(func() bool {
				...
				return true
			}, "30s", "5s").Should(BeTrue(), "untracked rsyslog.service should not appear in status.systemd")

			// Clearing matchPatterns removes systemd tracking

			By("clearing device.spec.systemd.matchPatterns")
			err = harness.UpdateSystemdMatchPatterns(deviceId, []string{})
			Expect(err).ToNot(HaveOccurred())

			By("waiting for status.systemd to be empty after clearing matchPatterns (expected behavior)")
			harness.WaitForSystemdUnitsLen(deviceId, 0, TENMINTIMEOUT, TENSECTIMEOUT)
			...
			e2e.RestoreServiceOnDevice(harness, "chronyd.service")
			// Clean up the failing test service (don't try to restart it)
			_, _ = harness.VM.RunSSH([]string{"sudo", "systemctl", "disable", failingServiceName}, nil)
			_, _ = harness.VM.RunSSH([]string{"sudo", "rm", "-f", fmt.Sprintf("/etc/systemd/system/%s", failingServiceName)}, nil)
			_, _ = harness.VM.RunSSH([]string{"sudo", "systemctl", "daemon-reload"}, nil)
			e2e.RestoreServiceOnDevice(harness, untrackedService)
		})
