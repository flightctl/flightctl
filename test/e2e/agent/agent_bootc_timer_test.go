...
var _ = Describe("Bootc timer compliance", func() {
	It("should monitor bootc timer compliance and generate events/alerts", Label("bootc-timer", "sanity", "agent"), func() {
		...
		By("Waiting for agent to be active")
		Eventually(harness.IsAgentServiceRunning, "30s", "1s").Should(BeTrue())
		...
		if envType == "kind" || envType == "ocp" {
			...
			// Build properly encoded alert filter query
			params := url.Values{}
			params.Add("filter", fmt.Sprintf("org_id=%s", orgID))
			params.Add("filter", fmt.Sprintf("alertname=%s", alertNameBootcTimerNonCompliant))
			params.Add("filter", fmt.Sprintf("resource=%s", deviceName))
			alertsPath := alertmanagerAlertsPath + "?" + params.Encode()
