...
var _ = Describe("Template variables in the device configuraion", func() {
	...
	Context("parametrisable_templates", func() {
		It(`Verifies that Flightctl fleet resource supports parametrisable device
			...
			Expect(err).ToNot(HaveOccurred())
			err = CreateTestFleetWithConfig(harness, configProviderSpec)
			Expect(err).ToNot(HaveOccurred())
...
func CreateTestFleetWithConfig(harness *e2e.Harness, configProviderSpec v1alpha1.ConfigProviderSpec) error {
	var testFleetSpec = v1alpha1.DeviceSpec{
		Config: &[]v1alpha1.ConfigProviderSpec{
			configProviderSpec,
		},
	}
	err := harness.CreateOrUpdateTestFleet(fleetTestName, testFleetSelector, testFleetSpec)
	Expect(err).ToNot(HaveOccurred())
	return err
}
