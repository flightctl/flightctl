...
var _ = Describe("ImageBuild", Label("imagebuild"), func() {
	Context("ImageBuild and ImageExport negative API behavior", func() {
		...
		It("should fail ImageBuild after creation when destination push cannot authenticate", Label("88399", "imagebuild", "slow"), func() {
			...
			src, err := createQuaySourceRepository(workerHarness, testID, &negCleanup)
			Expect(err).ToNot(HaveOccurred())
			dst, err2 := createAuthenticatedRegistryDestRepository(workerHarness, testID, &negCleanup, "wrong-password-"+testID)
...
func createStandardImageBuildRepositories(h *e2e.Harness, testID string, c *negAPIResourceCleanup) (sourceRepoName, destRepoName string, err error) {
	sourceRepoName, err = createQuaySourceRepository(h, testID, c)
	if err != nil {
...
// createQuaySourceRepository creates a read-only OCI repository pointing at quay.io (or the OCP mirror),
// registering it for cleanup.
// Returns an error if the repository cannot be created.
func createQuaySourceRepository(h *e2e.Harness, testID string, c *negAPIResourceCleanup) (string, error) {
	if h == nil {
		return "", fmt.Errorf("createQuaySourceRepository: harness is nil")
	}
	resolveSourceRegistry(h)
	name := fmt.Sprintf("repo-src-%s", testID)
	GinkgoWriter.Printf("createQuaySourceRepository: creating source repo %q at %s\n", name, sourceRegistry)
	_, err := resources.CreateOCIRepository(h, name, sourceRegistry,
		lo.ToPtr(api.Https), lo.ToPtr(api.Read), isLocalSourceRegistry(), nil)
	if err != nil {
		return "", fmt.Errorf("createQuaySourceRepository %q: %w", name, err)
	}
	c.sourceRepo = name
	return name, nil
}
