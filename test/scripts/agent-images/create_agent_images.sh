...
# if BREW_BUILD_ID is provided, download RPMs from brew registry to bin/brew-rpm
if [ -n "${BREW_BUILD_ID:-}" ]; then
    echo "Using BREW_BUILD_ID=${BREW_BUILD_ID} for brew registry RPMs"

    # Download required RPMs using shared function
    if ! download_brew_rpms "bin/brew-rpm" "${BREW_BUILD_ID}" "flightctl-agent-*" "flightctl-selinux*"; then
        exit 1
    fi

    BUILD_ARGS="--build-arg=BREW_BUILD_ID=${BREW_BUILD_ID}"
    ...
    echo "No BREW_BUILD_ID or FLIGHTCTL_RPM provided, using local RPMs only"
    BUILD_ARGS=""
fi
