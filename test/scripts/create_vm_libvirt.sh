...
if [ "${USE_CUSTOM_QEMU}" = "true" ]; then
    ...
    # sync ensures the cloud-init.disabled file is flushed to disk before
    # virsh destroy, which is an immediate power-off (no filesystem flush).
    ssh -i ${SSH_PRIVATE_KEY_PATH} -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
        -o BatchMode=yes ${USER}@${VM_DEFAULT_IP} "sudo touch /etc/cloud/cloud-init.disabled && sync"
...
if [ -n "${TPM_PASSTHROUGH_ARGS}" ]; then
    ...
    # Replace the VM's qemu-kvm with the custom TPM passthrough build.
    # The custom binary is built with --enable-tpm which CentOS/RHEL qemu-kvm
    # does not include. After replacing, we must:
    #   - Set root ownership (scp preserves the sending user's ownership)
    #   - Restore the SELinux context to qemu_exec_t (libvirt refuses non-labeled binaries)
    #   - Symlink firmware/ROM files into /usr/share/qemu/ because the custom binary
    #     resolves its datadir relative to its own path (/usr/libexec/../share/qemu/)
    #     which differs from where the system packages install them:
    #       /usr/local/qemu-tpm/share/qemu/ - BIOS, keymaps from the custom build
    #       /usr/share/qemu-kvm/            - kvmvapic.bin and other system ROMs
    #       /usr/share/ipxe/qemu/           - efi-virtio.rom and other PXE ROMs
    if [ -f "${QEMU_VM_BINARY}" ]; then
        ...
        ssh -i ${SSH_PRIVATE_KEY_PATH} -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
            ${USER}@${VM_DEFAULT_IP} "sudo cp /usr/libexec/qemu-kvm /usr/libexec/qemu-kvm.orig && sudo mv /tmp/qemu-system-x86_64 /usr/libexec/qemu-kvm && sudo chown root:root /usr/libexec/qemu-kvm && sudo chmod 755 /usr/libexec/qemu-kvm && sudo restorecon /usr/libexec/qemu-kvm && sudo mkdir -p /usr/share/qemu && sudo ln -sf /usr/local/qemu-tpm/share/qemu/* /usr/share/qemu/ && sudo ln -sf /usr/share/qemu-kvm/* /usr/share/qemu/ && sudo ln -sf /usr/share/ipxe/qemu/* /usr/share/qemu/"
    ...
    # Configure TPM device access inside the VM for the e2e test user.
    # TPM passthrough requires exclusive access to /dev/tpm0 (not /dev/tpmrm0).
    # See test/e2e/tpm/README.md for details on why /dev/tpmrm0 cannot be used.
    echo "Configuring TPM device access inside the VM..."
    ssh -i ${SSH_PRIVATE_KEY_PATH} -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
        ${USER}@${VM_DEFAULT_IP} bash -s <<'TPM_SETUP_EOF'
set -e
# tpm2-abrmd holds /dev/tpm0 open which blocks exclusive passthrough access.
sudo systemctl stop tpm2-abrmd 2>/dev/null || true
sudo systemctl disable tpm2-abrmd 2>/dev/null || true

# Default udev rules only grant /dev/tpm0 access to the tss user, not the group.
# Add a rule that sets group read/write so members of the tss group can access it.
...
# Add the current user to the tss group so libvirt (session mode) can access /dev/tpm0.
# Group membership takes effect on the next login (SSH session).
