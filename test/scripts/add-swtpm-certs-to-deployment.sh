...
if [ "$#" -gt 1 ]; then
    echo "Usage: $0 [swtpm-cert-directory]"
    echo "  swtpm-cert-directory: Directory containing TPM CA certificates (default: /var/lib/swtpm-localca)"
    exit 1
fi
SWTPM_CERT_DIR=${1:-${SWTPM_CERT_DIR:-/var/lib/swtpm-localca}}
NAMESPACE=${NAMESPACE:-flightctl-external}
CONFIG_MAP_NAME="tpm-ca-certs"

echo "Checking for swtpm CA certificates in ${SWTPM_CERT_DIR}..."

# Find all files ending with *cert.pem
echo "Searching for certificates matching pattern: *cert.pem"
mapfile -t CERT_FILES < <(find "${SWTPM_CERT_DIR}" -maxdepth 1 -type f -name "*cert.pem" 2>/dev/null || true)

if [ ${#CERT_FILES[@]} -eq 0 ]; then
    echo "ERROR: No swtpm CA certificates (*cert.pem) found in ${SWTPM_CERT_DIR}."
    echo "Please install swtpm-tools and ensure the CA is generated (e.g., by running 'swtpm_localca')."
    exit 1
fi
...
if [ -z "$EXISTING_KEYS" ]; then
    ...
    if [ ${#NEW_CERT_FILES[@]} -gt 0 ]; then
        ...
        echo "All local swtpm certificates are already in the ConfigMap. No changes needed."
        exit 0
    fi
fi
...
echo "Successfully configured deployment with swtpm CA certificates."
