...
if [[ "${GINKGO_TOTAL_NODES}" -gt 1 ]]; then
    ...
    # Export the test list for the custom reporter to categorize skipped tests
    if [[ -s "${NODE_TESTS}" ]]; then
        # Read all tests, escape regex metacharacters, and join them with | for regex OR
        # Create a delimited string of tests assigned to this node (for reporter categorization)
        NODE_TESTS_PATTERN=$(sed 's/[[\.*^$()+?{|\\]/\\&/g' "${NODE_TESTS}" | paste -sd '|')
        export FLIGHTCTL_NODE_TESTS="${NODE_TESTS_PATTERN}"
        echo "Exported node test pattern for custom reporter: ${NODE_TESTS_PATTERN}"
    fi
...
# Use focus flag if set by user (but not if it was auto-generated by distributed mode)
if [[ -n "${GINKGO_FOCUS}" ]]; then
