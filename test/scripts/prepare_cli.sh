...
if [[ -n "${BREW_BUILD_ID:-}" ]]; then
    echo -e "\e[32mInstalling the CLI from brew registry BREW_BUILD_ID: ${BREW_BUILD_ID}\e[0m"

    # Download all RPMs using shared function
    if ! download_brew_rpms "bin/brew-rpm" "${BREW_BUILD_ID}"; then
        exit 1
    fi
    ...
    if [[ -z "${CLI_RPM}" ]]; then
        echo "ERROR: No flightctl CLI RPM found in brew build ${BREW_BUILD_ID}"
        echo "Available RPMs:"
