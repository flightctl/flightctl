...
build_prebase_image() {
    local image_name="${IMAGE_REPO}:prebase-10-${TAG}"
    ...
    podman build "${CACHE_FLAGS[@]}" "${PODMAN_BUILD_FLAGS[@]}" ${PODMAN_BUILD_EXTRA_FLAGS:-} \
        ...
        --target prebase \
        -f "${SCRIPT_DIR}/Containerfile-base10" \
        -t "${image_name}" -t "${IMAGE_REPO}:prebase-10" \
        .
    echo -e "\033[32mSuccessfully built ${image_name}\033[m"
}

build_base_image() {
    local final_image="${IMAGE_REPO}:base-10-${TAG}"
    local prebase="${PREBASE_IMAGE:-}"

    if [ -z "${prebase}" ]; then
        prebase="${IMAGE_REPO}:prebase-10-${TAG}"
    ...
    if [ -n "${prebase}" ]; then
        ...
        podman build "${CACHE_FLAGS[@]}" "${PODMAN_BUILD_FLAGS[@]}" ${PODMAN_BUILD_EXTRA_FLAGS:-} \
            ...
            -f "${SCRIPT_DIR}/Containerfile-base10" \
            -t "${final_image}" -t "${IMAGE_REPO}:base-10" \
            .
    else
        podman build "${CACHE_FLAGS[@]}" "${PODMAN_BUILD_FLAGS[@]}" ${PODMAN_BUILD_EXTRA_FLAGS:-} \
            --build-arg SOURCE_GIT_TAG="${SOURCE_GIT_TAG}" \
