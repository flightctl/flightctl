...
_run_template_migration:
	@MIGRATION_IMAGE="$(MIGRATION_IMAGE)" bash -euo pipefail -c '\
	  ...
	  if [ -n "$$MIGRATION_IMAGE" ]; then \
	    ...
	    echo "##################################################"; \
	    $(MAKE) --no-print-directory -B flightctl-db-setup-container FLAVOR=$(FLAVOR); \
...
prepare-e2e-test: $(FLAVORCTL)
prepare-e2e-test: RPM_MOCK_ROOT=$(shell $(FLAVORCTL) get $(FLAVOR) test.rpm_mock_root)
prepare-e2e-test: AGENT_OS_ID=$(shell $(FLAVORCTL) get $(FLAVOR) test.agent_os_id)
