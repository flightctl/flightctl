...
// GetDefaultK8sAdminContext returns the current kubectl context (cluster-admin / default e2e context).
...
func (h *Harness) GetK8sApiEndpoint(ctx context.Context, k8sContext string) (string, error) {
	_ = ctx // kubectl subprocess does not accept cancellation today
	if h == nil {
		return "", fmt.Errorf("harness is nil")
	}
	if k8sContext == "" {
	...
	if err != nil {
		return "", fmt.Errorf("kubectl config view api server for context %q: %w", k8sContext, err)
	}
	server := strings.TrimSpace(out)
	if server == "" {
		return "", fmt.Errorf("empty API server in kubeconfig for context %q", k8sContext)
	}
	return server, nil
}

// ChangeK8sContext runs kubectl config use-context.
func (h *Harness) ChangeK8sContext(ctx context.Context, k8sContext string) (string, error) {
	...
	out, err := h.SH("kubectl", "config", "use-context", k8sContext)
	if err != nil {
		return "", fmt.Errorf("kubectl config use-context %q: %w", k8sContext, err)
	}
