...
// containerCleanupTimeout bounds teardown when a device setup has already failed or timed out.
...
type ContainerDeviceConfig struct {
	// Name is the container name; it must be unique per device instance.
	Name string
...
// Run starts the device container if it hasn't been created, or restarts it if it was stopped.
func (c *ContainerDevice) Run() error {
	return c.RunContext(context.Background())
}

// RunContext is the context-aware form of Run. It allows callers to cancel container creation
// and restart operations when setup times out.
func (c *ContainerDevice) RunContext(ctx context.Context) error {
	...
	if c.container != nil {
		...
		if err := c.container.Start(ctx); err != nil {
			return fmt.Errorf("failed to restart device container %s: %w", c.cfg.Name, err)
		}
		c.started = true
		return nil
	}

	// A container from a previous, uncleanly-terminated process may still hold this name. Keep
	// both the runtime CLI probe and removal within the caller's setup deadline.
	exists, err := containers.ContainerExistsByNameContext(ctx, c.cfg.Name)
	if err != nil {
		return fmt.Errorf("failed to check for stale device container %s: %w", c.cfg.Name, err)
	}
	if exists {
		if err := containers.RemoveContainerByNameContext(ctx, c.cfg.Name); err != nil {
			return fmt.Errorf("failed to remove stale device container %s: %w", c.cfg.Name, err)
		}
	}
	if err := ctx.Err(); err != nil {
		return err
	...
	if err := c.updateCATrustLocked(ctx); err != nil {
...
func (c *ContainerDevice) updateCATrustLocked(ctx context.Context) error {
	hasCA := false
	for _, f := range c.cfg.Files {
...
func (c *ContainerDevice) RunAndWaitForSSH() error {
	return c.RunAndWaitForSSHContext(context.Background())
}

// RunAndWaitForSSHContext is the context-aware form of RunAndWaitForSSH.
func (c *ContainerDevice) RunAndWaitForSSHContext(ctx context.Context) error {
	if err := c.RunContext(ctx); err != nil {
		return err
	}
	return c.WaitForSSHToBeReadyContext(ctx)
}

// WaitForSSHToBeReady polls until exec into the container succeeds. The initial GenericStart wait
// strategy already blocks until flightctl-agent.service is active, so this is normally
// near-instant; it is also useful after starting an existing stopped container.
func (c *ContainerDevice) WaitForSSHToBeReady() error {
	return c.WaitForSSHToBeReadyContext(context.Background())
}

// WaitForSSHToBeReadyContext polls until exec into the container succeeds, bounded by both the
// usual readiness timeout and the caller's context.
func (c *ContainerDevice) WaitForSSHToBeReadyContext(ctx context.Context) error {
	...
	for time.Now().Before(deadline) {
		...
		if ctxErr := ctx.Err(); ctxErr != nil {
			return fmt.Errorf("device container %s readiness canceled: %w", c.cfg.Name, ctxErr)
		}
		lastErr = err
		...
		case <-ctx.Done():
			timer.Stop()
			return fmt.Errorf("device container %s readiness canceled: %w", c.cfg.Name, ctx.Err())
		case <-timer.C:
		}
	}
	if err := ctx.Err(); err != nil {
...
func (c *ContainerDevice) ForceDelete() error {
	...
	defer cancel()
	return c.ForceDeleteContext(ctx)
}

// ForceDeleteContext stops and removes the container, bounded by ctx. If removal fails, the
// container handle remains available for a later cleanup retry.
func (c *ContainerDevice) ForceDeleteContext(ctx context.Context) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	ct := c.container
	if ct == nil {
		if err := removeContainerByNameContext(ctx, c.cfg.Name); err != nil {
			return err
		}
		c.started = false
		return nil
	...
	if err := ct.Terminate(ctx); err != nil {
		terminateErr := fmt.Errorf("failed to terminate device container %s: %w", c.cfg.Name, err)
		if ctxErr := ctx.Err(); ctxErr != nil {
			return errors.Join(terminateErr, ctxErr)
		}
		if cleanupErr := removeContainerByNameContext(ctx, c.cfg.Name); cleanupErr != nil {
			return errors.Join(terminateErr, cleanupErr)
		}
	}
	c.container = nil
	c.started = false
...
func removeContainerByNameContext(ctx context.Context, name string) error {
	exists, err := containers.ContainerExistsByNameContext(ctx, name)
	if err != nil {
		return fmt.Errorf("failed to check whether device container %s exists: %w", name, err)
	}
	if !exists {
		return nil
	}
	if err := containers.RemoveContainerByNameContext(ctx, name); err != nil {
		if ctxErr := ctx.Err(); ctxErr != nil {
			return fmt.Errorf("failed to remove device container %s before cleanup deadline: %w", name, ctxErr)
		}
		// A concurrent cleanup may have removed it after the existence probe; treat that as success.
		exists, checkErr := containers.ContainerExistsByNameContext(ctx, name)
		if checkErr == nil && !exists {
			return nil
		}
		if checkErr != nil {
			err = errors.Join(err, fmt.Errorf("failed to recheck device container %s: %w", name, checkErr))
		}
		return fmt.Errorf("failed to remove device container %s: %w", name, err)
	}
	return nil
}
