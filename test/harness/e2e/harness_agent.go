...
func (h *Harness) PrepareVMAccess(deviceID, artifactDir string) error {
	...
	if err != nil {
		_ = WriteEvidence(artifactDir, "vm_ssh_whoami.txt", "whoami", sshWhoamiOut.String(), err)
		return err
	}
	if err := WriteEvidence(artifactDir, "vm_ssh_whoami.txt", "whoami", sshWhoamiOut.String(), nil); err != nil {
		return err
	}
