...
func (h *Harness) AddLabelToFleet(fleetName string, labelKey, labelValue string) {
	h.UpdateFleetWithRetries(fleetName, func(fleet *v1alpha1.Fleet) {
		if fleet.Metadata.Labels == nil {
			labels := make(map[string]string)
			fleet.Metadata.Labels = &labels
		}
		(*fleet.Metadata.Labels)[labelKey] = labelValue
	})
}
