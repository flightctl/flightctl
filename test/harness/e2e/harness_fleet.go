...
// HavingReason returns a type-safe Gomega matcher for the Condition's Reason field.
func HavingReason(expected string) types.GomegaMatcher {
...
// HavingStatus returns a type-safe Gomega matcher for the Condition's Status field.
func HavingStatus(expected v1alpha1.ConditionStatus) types.GomegaMatcher {
	return WithTransform(
...
// HavingType returns a type-safe Gomega matcher for the Condition's Type field.
func HavingType(expected v1alpha1.ConditionType) types.GomegaMatcher {
	return WithTransform(
		// This part is also checked by the compiler!
...
func (h *Harness) WaitForFleetUpdateToFail(fleetName string) {
	...
	Eventually(h.GetRolloutStatus, LONGTIMEOUT, POLLING).
		...
		Should(SatisfyAll(
			HavingType(v1alpha1.ConditionTypeFleetRolloutInProgress),
			HavingStatus(v1alpha1.ConditionStatusFalse),
			HavingReason(v1alpha1.RolloutSuspendedReason),
