...
func (h *Harness) WaitForDeviceOnlineStatus(enrollmentID string) (string, *v1alpha1.Device, error) {
	...
	Eventually(func() error {
		...
		// Check for TPM-specific auth method
		if device.Status.AuthenticationMethod != "tpm" {
			return fmt.Errorf("expected TPM authentication method, got: %s", device.Status.AuthenticationMethod)
		}

		// Verify device is online and healthy
		if device.Status.Summary == nil ||
