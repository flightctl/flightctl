...
func TestBroadcastSubscribe(t *testing.T) {
	...
	provider, err := NewRedisProvider(context.Background(), logrus.New(), "localhost", 6379, "")
	if err != nil {
	...
	t.Run("MultipleSubscribers", func(t *testing.T) {
		...
		broadcaster, err := provider.NewBroadcaster(channelName)
		require.NoError(t, err)
		defer broadcaster.Close()

		err = broadcaster.Broadcast(ctx, testMessage)
		require.NoError(t, err)
		...
		for i, received := range receivedMessages {
			assert.Equal(t, testMessage, received, "Subscriber %d should have received the message", i)
		}
		mu.Unlock()
	})
	...
	t.Run("LateSubscriber", func(t *testing.T) {
		...
		broadcaster, err := provider.NewBroadcaster(channelName + "-late")
		require.NoError(t, err)
		defer broadcaster.Close()

		err = broadcaster.Broadcast(ctx, []byte("early message"))
		require.NoError(t, err)
		...
		subscriber, err := provider.NewSubscriber(channelName + "-late")
		require.NoError(t, err)
		defer subscriber.Close()
	...
	t.Run("HandlerError", func(t *testing.T) {
		...
		subscriber, err := provider.NewSubscriber(channelName + "-error")
		require.NoError(t, err)
		defer subscriber.Close()

		broadcaster, err := provider.NewBroadcaster(channelName + "-error")
		require.NoError(t, err)
		defer broadcaster.Close()
		...
		err = broadcaster.Broadcast(ctx, []byte("error message"))
		require.NoError(t, err)
...
func TestBroadcastSubscribeClosedChannel(t *testing.T) {
	...
	broadcaster, err := provider.NewBroadcaster("test-closed-channel")
	require.NoError(t, err)

	subscriber, err := provider.NewSubscriber("test-closed-channel")
	require.NoError(t, err)
	...
	err = broadcaster.Broadcast(context.Background(), []byte("test"))
...
func TestMultipleSubscriptionsFromSameSubscriber(t *testing.T) {
	...
	subscriber, err := provider.NewSubscriber("test-multiple-subscriptions")
	require.NoError(t, err)
	defer subscriber.Close()
	...
	// Create broadcaster and send message
	broadcaster, err := provider.NewBroadcaster("test-multiple-subscriptions")
	require.NoError(t, err)
	defer broadcaster.Close()
