...
func TestPublishSubscribe(t *testing.T) {
	...
	t.Run("MultipleSubscribers", func(t *testing.T) {
		...
		for i := 0; i < numSubscribers; i++ {
			idx := i
			subscriber, err := provider.NewPubSubSubscriber(channelName)
		...
		broadcaster, err := provider.NewPubSubPublisher(channelName)
		require.NoError(t, err)
	...
	t.Run("LateSubscriber", func(t *testing.T) {
		...
		broadcaster, err := provider.NewPubSubPublisher(channelName + "-late")
		require.NoError(t, err)
		defer broadcaster.Close()
		...
		subscriber, err := provider.NewPubSubSubscriber(channelName + "-late")
		require.NoError(t, err)
		defer subscriber.Close()
	...
	t.Run("HandlerError", func(t *testing.T) {
		...
		subscriber, err := provider.NewPubSubSubscriber(channelName + "-error")
		require.NoError(t, err)
		defer subscriber.Close()

		broadcaster, err := provider.NewPubSubPublisher(channelName + "-error")
		require.NoError(t, err)
		defer broadcaster.Close()
...
func TestBroadcastSubscribeClosedChannel(t *testing.T) {
	...
	broadcaster, err := provider.NewPubSubPublisher("test-closed-channel")
	require.NoError(t, err)

	subscriber, err := provider.NewPubSubSubscriber("test-closed-channel")
	require.NoError(t, err)
...
func TestMultipleSubscriptionsFromSameSubscriber(t *testing.T) {
	...
	subscriber, err := provider.NewPubSubSubscriber("test-multiple-subscriptions")
	require.NoError(t, err)
	defer subscriber.Close()
	...
	// Create broadcaster and send message
	broadcaster, err := provider.NewPubSubPublisher("test-multiple-subscriptions")
	require.NoError(t, err)
	defer broadcaster.Close()
