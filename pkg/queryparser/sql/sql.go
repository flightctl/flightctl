...
func withKeyArg() verificationHandler {
	return func(queryFunc *queryparser.QueryFunc) error {
		...
		if firstArgName == "CAST" {
			return withKeyArg()(args[0].(*queryparser.QueryArgFunc).Value())
		}
...
func withKeyOrValueArg() verificationHandler {
	return func(queryFunc *queryparser.QueryFunc) error {
		args := queryFunc.Args()
		if len(args) == 0 {
			return fmt.Errorf("no arguments specified for function %s", queryFunc.Name())
...
func NewSQLParser(options ...SQLParserOption) (queryparser.Parser, error) {
	...
	sp.funcs = map[string]handler{
		...
		"K": {
			usedBy: queryparser.NewSet[string]().Add("EQ", "NEQ", "LT", "LTE", "GT", "GTE", "IN", "NOTIN", "LIKE",
				"NLIKE", "ANY", "NANY", "ALL", "NALL", "OVERLAPS", "NOVERLAPS", "CONTAINS", "NCONTAINS", "ISNULL", "ISNOTNULL", "CAST"),
