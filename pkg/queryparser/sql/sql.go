...
func withFuncVAndValues() verificationHandler {
	return func(qf *queryparser.QueryFunc) error {
		args := qf.Args()
		if len(args) == 0 {
			return fmt.Errorf("no arguments specified for function %s", qf.Name())
		...
		for _, arg := range args[1:] {
			if !queryparser.IsValue(arg) {
				return fmt.Errorf("all arguments after the first must be a value type for function %s", qf.Name())
			}
		}
		return nil
	}
...
func NewSQLParser(options ...SQLParserOption) (queryparser.Parser, error) {
	...
	sp.funcs = map[string]handler{
		...
		"EQ": {
			...
			Verifications: []verificationHandler{withKeyArg(), withNoValues()},
		...
		"CAST": {
			...
			Verifications: []verificationHandler{withFuncVAndValues()},
			handle:        sp.queryCast,
		},
		"K": {
			usedBy: queryparser.NewSet[string]().Add("EQ", "NEQ", "LT", "LTE", "GT", "GTE", "IN", "NOTIN", "LIKE",
