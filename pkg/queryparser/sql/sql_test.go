...
func TestSQLQueries(t *testing.T) {
	...
	*/
	testGoodQueries := map[string][]string{
		...
		"ANY(K(a),V(1))":                                 {"? = ANY(a)", "1"},
		"NOTANY(K(a),V(1))":                              {"? != ANY(a)", "1"},
		"ALL(K(a),V(1),V(2),V(3))":                       {"ARRAY[?, ?, ?] <@ a", "1", "2", "3"},
		"NOTALL(K(a),V(1),V(2),V(3))":                    {"NOT (ARRAY[?, ?, ?] <@ a)", "1", "2", "3"},
	...
	testBadQueries := []string{
		...
		// ANY and NOTANY require exactly two queries parameters
		"ANY()",                  // No arguments
		"ANY(K(a))",              // One argument
		"ANY(K(a),V(b),V(c))",    // Three arguments
		"ANY(V(b),K(a))",         // First query must be K or CAST
		"ANY(val2,val2)",         // First query must be K or CAST
		"ANY(K(b),val)",          // No values
		"NOTANY()",               // No arguments
		"NOTANY(K(a))",           // One argument
		"NOTANY(K(a),V(b),V(c))", // Three arguments
		"NOTANY(V(b),K(a))",      // First query must be K or CAST
		"NOTANY(val2,val2)",      // First query must be K or CAST
		"NOTANY(K(b),val)",       // No values

		// ALL and NOTALL require at least two queries parameters
		"ALL()",             // No arguments
		"ALL(K(a))",         // One argument
		"ALL(V(b),K(a))",    // First query must be K or CAST
		"ALL(val2,val2)",    // First query must be K or CAST
		"ALL(K(b),val)",     // No values
		"NOTALL()",          // No arguments
		"NOTALL(K(a))",      // One argument
		"NOTALL(V(b),K(a))", // First query must be K or CAST
		"NOTALL(val2,val2)", // First query must be K or CAST
		"NOTALL(K(b),val)",  // No values
		...
		"NOTANY(EQ(K(a), V(1)))",          // EQ cannot be nested within NOTANY
		...
		"ANY(OR(K(a), V(1)))",             // OR cannot be nested within ANY
		...
		"OVERLAPS(ALL(K(a), K(b)))",       // ALL cannot be nested within OVERLAPS
		"NOTALL(OR(K(a), K(b)))",          // OR cannot be nested within NOTALL
