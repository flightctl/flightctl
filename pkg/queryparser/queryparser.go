...
func Tokenize(ctx context.Context, input string) (TokenSet, error) {
	...
	for i, char := range input {
		...
		case ',':
			if currentStr := strings.TrimSpace(current.String()); currentStr != "" {
				if openParenCount == 0 {
					return nil, fmt.Errorf("value without a function at position %d", pos)
				}
				tokens = append(tokens, Token{Type: TokenValue, Value: currentStr})
				current.Reset()
			} else if !expectComma {
			...
			expectComma = false // After a comma, expect a new function or a value
		default:
	...
	for i, token := range tokens {
		...
		case TokenFunc:
			...
			if currentFunc.UsedBy.Size() > 0 {
				parentFunc := RootFunc
				if len(opStack) > 0 {
					// Safe to access because the stack contains only processed data
