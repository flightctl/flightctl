...
create_secrets() {
    if [ -z "${FLIGHTCTL_POSTGRESQL_PASSWORD}" ]; then
        FLIGHTCTL_POSTGRESQL_PASSWORD=$(generate_secret)
    fi
    if [ -z "${FLIGHTCTL_POSTGRESQL_MASTER_PASSWORD}" ]; then
        FLIGHTCTL_POSTGRESQL_MASTER_PASSWORD=$(generate_secret)
    fi
    if [ -z "${FLIGHTCTL_POSTGRESQL_USER_PASSWORD}" ]; then
        FLIGHTCTL_POSTGRESQL_USER_PASSWORD=$(generate_secret)
    fi
    if [ -z "${FLIGHTCTL_KV_PASSWORD}" ]; then
        FLIGHTCTL_KV_PASSWORD=$(generate_secret)
    fi
