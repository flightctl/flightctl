...
# Ensure optional delta repository secrets and their worker drop-in are ready.
ensure_delta_generation_secrets() {
    ...
    if [[ -z "${DELTA_GENERATION_DEFAULT_REPOSITORY_USERNAME:-}" ||
        ...
        if sudo podman secret exists "$username_secret" && sudo podman secret exists "$password_secret"; then
            create_delta_generation_dropin "$dropin_dir" "$dropin_file" "$username_secret" "$password_secret"
...
# Create the deployment-managed worker drop-in.
create_delta_generation_dropin() {
    local dropin_dir="$1"
    local dropin_file="$2"
