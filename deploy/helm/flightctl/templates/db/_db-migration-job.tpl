    ...
    spec:
      ...
      - name: setup-database-users
        ...
        - name: DB_ADMIN_USER
          valueFrom:
            secretKeyRef:
              name: {{ default "flightctl-db-admin-secret" $ctx.Values.db.masterUserSecretName }}
              key: masterUser
        ...
        - name: DB_MIGRATION_USER
          valueFrom:
            secretKeyRef:
              name: {{ default "flightctl-db-migration-secret" $ctx.Values.db.migrationUserSecretName }}
              key: migrationUser
        ...
        - name: DB_APP_USER
          valueFrom:
            secretKeyRef:
              name: {{ default "flightctl-db-app-secret" $ctx.Values.db.applicationUserSecretName }}
              key: user
      ...
      - name: run-migrations
        ...
        {{- end }}
        {{- if $ctx.Values.db.external.sslSecretName }}
        - name: PGSSLCERT
          value: /etc/ssl/postgres/client-cert.pem
        - name: PGSSLKEY
          value: /etc/ssl/postgres/client-key.pem
        {{- end }}
        {{- if $ctx.Values.db.external.sslConfigMapName }}
        - name: PGSSLROOTCERT
