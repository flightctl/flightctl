...
| imageBuilderWorker.serviceImages | object | `{"bootcImageBuilder":{"image":"","skipTlsVerify":false},"podman":{"image":"","skipTlsVerify":false},"pullSecretName":"","rhelBootcImageBuilder":{"image":"","skipTlsVerify":false},"syft":{"image":"","skipTlsVerify":false}}` | Builder images (podman, bootc-image-builder, syft) and skip-TLS options |
...
| imageBuilderWorker.serviceImages.rhelBootcImageBuilder.image | string | `""` | RHEL-specific bootc-image-builder image, used automatically for RHEL 10+ source images. Leave empty to use the default (registry.redhat.io/rhel10/bootc-image-builder:latest). Requires registry.redhat.io credentials via pullSecretName. |
| imageBuilderWorker.serviceImages.rhelBootcImageBuilder.skipTlsVerify | bool | `false` | Set to true to skip TLS verification when pulling the RHEL bootc-image-builder image. |
