...
versions=$(find "$REPO_OUTPUT_DIR" -name "*.rpm" -exec basename {} \; | \
    sed -E 's/.*-([0-9]+\.[0-9]+\.[0-9]+[^-]*)-[0-9]+\..*/\1/' | \
    sort -V | uniq | tr '\n' ' ')
...
cat >> "$REPO_OUTPUT_DIR/index.html" << EOF
    ...
    <div class="footer">
        ...
        <p>🏗️ <strong>Source:</strong> <a href="https://github.com/$REPO_OWNER/$REPO_NAME">$REPO_OWNER/$REPO_NAME</a></p>
    </div>
</body>
</html>
EOF
