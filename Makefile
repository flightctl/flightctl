...
push-containers: login
	...
	podman images --filter "reference=flightctl-*-$(OS):latest" --format "{{.Repository}}:{{.Tag}}" | xargs -r podman push
...
clean-containers:
	- podman images --filter "reference=flightctl-*" --format "{{.Repository}}:{{.Tag}}" | xargs -r podman rmi || true
