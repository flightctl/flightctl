...
flightctl-api-container: packaging/images/$(OS)/Containerfile.api $(FLAVOR_BUILDARGS) go.mod go.sum $(GO_FILES)
	podman build $(call CACHE_FLAGS_FOR_IMAGE,flightctl-api) \
		...
		-t localhost/flightctl-api:latest \
		-t localhost/flightctl-api-$(RHEL_OS):latest \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-api:$(SOURCE_GIT_TAG) \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-api-$(RHEL_OS):$(SOURCE_GIT_TAG) .
...
flightctl-pam-issuer-container: packaging/images/$(OS)/Containerfile.pam-issuer $(FLAVOR_BUILDARGS) go.mod go.sum $(GO_FILES)
	podman build $(call CACHE_FLAGS_FOR_IMAGE,flightctl-pam-issuer) \
		...
		-t localhost/flightctl-pam-issuer:latest \
		-t localhost/flightctl-pam-issuer-$(RHEL_OS):latest \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-pam-issuer:$(SOURCE_GIT_TAG) \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-pam-issuer-$(RHEL_OS):$(SOURCE_GIT_TAG) .
...
flightctl-db-setup-container: packaging/images/$(OS)/Containerfile.db-setup $(FLAVOR_BUILDARGS) deploy/scripts/setup_database_users.sh deploy/scripts/setup_database_users.sql
	podman build $(call CACHE_FLAGS_FOR_IMAGE,flightctl-db-setup) \
		...
		-t localhost/flightctl-db-setup:latest \
		-t localhost/flightctl-db-setup-$(RHEL_OS):latest \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-db-setup:$(SOURCE_GIT_TAG) \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-db-setup-$(RHEL_OS):$(SOURCE_GIT_TAG) .
...
flightctl-worker-container: packaging/images/$(OS)/Containerfile.worker $(FLAVOR_BUILDARGS) go.mod go.sum $(GO_FILES)
	podman build $(call CACHE_FLAGS_FOR_IMAGE,flightctl-worker) \
		...
		-t localhost/flightctl-worker:latest \
		-t localhost/flightctl-worker-$(RHEL_OS):latest \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-worker:$(SOURCE_GIT_TAG) \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-worker-$(RHEL_OS):$(SOURCE_GIT_TAG) .
...
flightctl-periodic-container: packaging/images/$(OS)/Containerfile.periodic $(FLAVOR_BUILDARGS) go.mod go.sum $(GO_FILES)
	podman build $(call CACHE_FLAGS_FOR_IMAGE,flightctl-periodic) \
		...
		-t localhost/flightctl-periodic:latest \
		-t localhost/flightctl-periodic-$(RHEL_OS):latest \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-periodic:$(SOURCE_GIT_TAG) \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-periodic-$(RHEL_OS):$(SOURCE_GIT_TAG) .
...
flightctl-alert-exporter-container: packaging/images/$(OS)/Containerfile.alert-exporter $(FLAVOR_BUILDARGS) go.mod go.sum $(GO_FILES)
	podman build $(call CACHE_FLAGS_FOR_IMAGE,flightctl-alert-exporter) \
		...
		-t localhost/flightctl-alert-exporter:latest \
		-t localhost/flightctl-alert-exporter-$(RHEL_OS):latest \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-alert-exporter:$(SOURCE_GIT_TAG) \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-alert-exporter-$(RHEL_OS):$(SOURCE_GIT_TAG) .
...
flightctl-alertmanager-proxy-container: packaging/images/$(OS)/Containerfile.alertmanager-proxy $(FLAVOR_BUILDARGS) go.mod go.sum $(GO_FILES)
	podman build $(call CACHE_FLAGS_FOR_IMAGE,flightctl-alertmanager-proxy) \
		...
		-t localhost/flightctl-alertmanager-proxy:latest \
		-t localhost/flightctl-alertmanager-proxy-$(RHEL_OS):latest \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-alertmanager-proxy:$(SOURCE_GIT_TAG) \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-alertmanager-proxy-$(RHEL_OS):$(SOURCE_GIT_TAG) .
...
flightctl-multiarch-cli-container: packaging/images/$(OS)/Containerfile.cli-artifacts $(FLAVOR_BUILDARGS) go.mod go.sum $(GO_FILES)
	podman build $(call CACHE_FLAGS_FOR_IMAGE,flightctl-cli-artifacts) \
		...
		-t localhost/flightctl-cli-artifacts:latest \
		-t localhost/flightctl-cli-artifacts-$(RHEL_OS):latest \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-cli-artifacts:$(SOURCE_GIT_TAG) \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-cli-artifacts-$(RHEL_OS):$(SOURCE_GIT_TAG) .
...
flightctl-userinfo-proxy-container: packaging/images/$(OS)/Containerfile.userinfo-proxy $(FLAVOR_BUILDARGS) go.mod go.sum $(GO_FILES)
	podman build $(call CACHE_FLAGS_FOR_IMAGE,flightctl-userinfo-proxy) \
		...
		-t localhost/flightctl-userinfo-proxy:latest \
		-t localhost/flightctl-userinfo-proxy-$(RHEL_OS):latest \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-userinfo-proxy:$(SOURCE_GIT_TAG) \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-userinfo-proxy-$(RHEL_OS):$(SOURCE_GIT_TAG) .
...
flightctl-telemetry-gateway-container: packaging/images/$(OS)/Containerfile.telemetry-gateway $(FLAVOR_BUILDARGS) go.mod go.sum $(GO_FILES)
	podman build $(call CACHE_FLAGS_FOR_IMAGE,flightctl-telemetry-gateway) \
		...
		-t localhost/flightctl-telemetry-gateway:latest \
		-t localhost/flightctl-telemetry-gateway-$(RHEL_OS):latest \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-telemetry-gateway:$(SOURCE_GIT_TAG) \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-telemetry-gateway-$(RHEL_OS):$(SOURCE_GIT_TAG) .
...
flightctl-imagebuilder-api-container: packaging/images/$(OS)/Containerfile.imagebuilder-api $(FLAVOR_BUILDARGS) go.mod go.sum $(GO_FILES)
	podman build $(call CACHE_FLAGS_FOR_IMAGE,flightctl-imagebuilder-api) \
		...
		-t localhost/flightctl-imagebuilder-api:latest \
		-t localhost/flightctl-imagebuilder-api-$(RHEL_OS):latest \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-imagebuilder-api:$(SOURCE_GIT_TAG) \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-imagebuilder-api-$(RHEL_OS):$(SOURCE_GIT_TAG) .
...
flightctl-imagebuilder-worker-container: packaging/images/$(OS)/Containerfile.imagebuilder-worker $(FLAVOR_BUILDARGS) go.mod go.sum $(GO_FILES)
	podman build $(call CACHE_FLAGS_FOR_IMAGE,flightctl-imagebuilder-worker) \
		...
		-t localhost/flightctl-imagebuilder-worker:latest \
		-t localhost/flightctl-imagebuilder-worker-$(RHEL_OS):latest \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-imagebuilder-worker:$(SOURCE_GIT_TAG) \
		-t $(REGISTRY)/$(REGISTRY_OWNER)/flightctl-imagebuilder-worker-$(RHEL_OS):$(SOURCE_GIT_TAG) .
