...
func NewBroadcaster(queuesProvider queues.Provider) (Publisher, error) {
	queuesPublisher, err := queuesProvider.NewPubSubPublisher(queueName)
	if err != nil {
		return nil, err
	}
	return &publisher{
...
func NewSubscriber(queuesProvider queues.Provider) (Subscriber, error) {
	subscriber, err := queuesProvider.NewPubSubSubscriber(queueName)
	if err != nil {
		return nil, err
	}
	return &consumer{
