...
func NewHandler(
	...
	go func() {
		...
		defer ticker.Stop()
		for range ticker.C {
			pamProvider.CleanupExpiredCodes()
			pamProvider.CleanupExpiredSessions()
		}
	}()
...
func (h *Handler) AuthLoginPost(w http.ResponseWriter, r *http.Request) {
	...
	redirectURL, err := h.pamProvider.Login(r.Context(), username, password, clientID, redirectURI, state)
	if err != nil {
		h.log.Errorf("Login failed for user %s: %v", username, err)
		// Return login form with error message
		loginForm := h.pamProvider.GetLoginFormWithError(clientID, redirectURI, state, "Invalid username or password")
		w.Header().Set("Content-Type", "text/html; charset=utf-8")
		w.WriteHeader(http.StatusOK)
		w.Write([]byte(loginForm))
		return
	...
	// Redirect to authorization endpoint with session
	h.log.Infof("Login successful for user %s, redirecting to %s", username, redirectURL)
...
func (h *Handler) AuthToken(w http.ResponseWriter, r *http.Request) {
	...
	tokenResponse, err := h.pamProvider.Token(r.Context(), &req)
	if err != nil {
