...
func createDevices(numDevices int, ctx context.Context, store Store, orgId uuid.UUID) {
	for i := 1; i <= numDevices; i++ {
		...
		_, err := store.Device().Create(ctx, orgId, &resource)
		if err != nil {
...
var _ = Describe("DeviceStore create", func() {
	...
	Context("Device store", func() {
		It("Get device success", func() {
			dev, err := store.Device().Get(ctx, orgId, "mydevice-1")
			Expect(err).ToNot(HaveOccurred())
			Expect(*dev.Metadata.Name).To(Equal("mydevice-1"))
		})

		It("Get device - not found error", func() {
			_, err := store.Device().Get(ctx, orgId, "nonexistent")
		...
		It("Get device - wrong org - not found error", func() {
			...
			_, err := store.Device().Get(ctx, badOrgId, "mydevice-1")
			Expect(err).To(HaveOccurred())
			Expect(err).To(Equal(gorm.ErrRecordNotFound))
		})

		It("Delete device success", func() {
			err := store.Device().Delete(ctx, orgId, "mydevice-1")
			Expect(err).ToNot(HaveOccurred())
		})

		It("Delete device success when not found", func() {
			err := store.Device().Delete(ctx, orgId, "nonexistent")
			Expect(err).ToNot(HaveOccurred())
		})

		It("Delete all devices in org", func() {
			otherOrgId, _ := uuid.NewUUID()
			err := store.Device().DeleteAll(ctx, otherOrgId)
			Expect(err).ToNot(HaveOccurred())

			listParams := ListParams{Limit: 1000}
			devices, err := store.Device().List(ctx, orgId, listParams)
			Expect(err).ToNot(HaveOccurred())
			Expect(len(devices.Items)).To(Equal(numDevices))

			err = store.Device().DeleteAll(ctx, orgId)
			Expect(err).ToNot(HaveOccurred())
		...
		It("List with paging", func() {
			listParams := ListParams{Limit: 1000}
			allDevices, err := store.Device().List(ctx, orgId, listParams)
			Expect(err).ToNot(HaveOccurred())
			...
			cont, err := ParseContinueString(devices.Metadata.Continue)
			Expect(err).ToNot(HaveOccurred())
