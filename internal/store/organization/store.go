...
// Ensure OrganizationStore implements the Store interface
var _ Store = (*OrganizationStore)(nil)

const externalIDIndex = "org_external_id_idx"

func NewOrganizationStore(db *gorm.DB) Store {
...
func (s *OrganizationStore) List(ctx context.Context, listParams store.ListParams) ([]*model.Organization, error) {
	db := s.getDB(ctx)

	resolver, err := selector.SelectorFieldResolver(&model.Organization{})
	if err != nil {
		return nil, err
	}

	if listParams.FieldSelector != nil {
		q, p, err := listParams.FieldSelector.Parse(ctx, resolver)
		if err != nil {
			return nil, err
		}
		db = db.Where(q, p...)
