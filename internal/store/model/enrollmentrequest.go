...
func NewEnrollmentRequestFromApiResource(resource *domain.EnrollmentRequest) (*EnrollmentRequest, error) {
	...
	return &EnrollmentRequest{
		Resource: Resource{
			...
			// A nil Annotations pointer (as set by SanitizeEnrollmentRequest for untrusted input)
			// means "don't touch annotations"; converting it to a nil map (not an empty one) lets
			// the store's nil-skip merge logic leave existing annotations untouched.
