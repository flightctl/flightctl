...
type FleetStore struct {
	...
	genericStore *GenericStore[*model.Fleet, model.Fleet, api.Fleet, *model.FleetList, api.FleetList]
}
...
func NewFleet(db *gorm.DB, log logrus.FieldLogger) Fleet {
	genericStore := NewGenericStore[*model.Fleet, model.Fleet, api.Fleet, *model.FleetList, api.FleetList](
		db,
		log,
		model.NewFleetFromApiResource,
		(*model.Fleet).ToApiResource,
		(*model.FleetList).ToApiResource,
...
func (s *FleetStore) List(ctx context.Context, orgId uuid.UUID, listParams ListParams, opts ...ListOption) (*api.FleetList, error) {
	...
	apiFleetList, _ := fleets.ToApiResource(nextContinue, numRemaining)
