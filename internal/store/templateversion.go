...
func (s *TemplateVersionStore) List(ctx context.Context, orgId uuid.UUID, listParams ListParams) (*api.TemplateVersionList, error) {
	...
	query, err := BuildBaseListQuery(s.db.Model(&templateVersions), orgId, listParams)
	if err != nil {
		return nil, err
	}
	if listParams.Limit > 0 {
	...
	if listParams.Limit > 0 && len(templateVersions) > listParams.Limit {
		...
		if listParams.Continue != nil {
			...
			countQuery, err := BuildBaseListQuery(s.db.Model(&templateVersions), orgId, listParams)
			if err != nil {
				return nil, err
			}
