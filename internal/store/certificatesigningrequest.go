...
func (s *CertificateSigningRequestStore) List(ctx context.Context, orgId uuid.UUID, listParams ListParams) (*api.CertificateSigningRequestList, error) {
	...
	query, err := BuildBaseListQuery(s.db.Model(&certificateSigningRequests), orgId, listParams)
	if err != nil {
		return nil, err
	}
	...
	if listParams.Limit > 0 && len(certificateSigningRequests) > listParams.Limit {
		...
		if listParams.Continue != nil {
			...
			countQuery, err := BuildBaseListQuery(s.db.Model(&certificateSigningRequests), orgId, listParams)
			if err != nil {
				return nil, err
			}
