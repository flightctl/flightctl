...
func createRepositories(numRepositories int, ctx context.Context, store Store, orgId uuid.UUID) {
	for i := 1; i <= numRepositories; i++ {
		...
		_, err := store.Repository().Create(ctx, orgId, &resource)
		if err != nil {
...
var _ = Describe("RepositoryStore create", func() {
	...
	Context("Repository store", func() {
		It("Get repository success", func() {
			dev, err := store.Repository().Get(ctx, orgId, "myrepository-1")
			Expect(err).ToNot(HaveOccurred())
			Expect(*dev.Metadata.Name).To(Equal("myrepository-1"))
		})

		It("Get repository - not found error", func() {
			_, err := store.Repository().Get(ctx, orgId, "nonexistent")
		...
		It("Get repository - wrong org - not found error", func() {
			...
			_, err := store.Repository().Get(ctx, badOrgId, "myrepository-1")
			Expect(err).To(HaveOccurred())
			Expect(err).To(Equal(gorm.ErrRecordNotFound))
		})

		It("Delete repository success", func() {
			err := store.Repository().Delete(ctx, orgId, "myrepository-1")
			Expect(err).ToNot(HaveOccurred())
		})

		It("Delete repository success when not found", func() {
			err := store.Repository().Delete(ctx, orgId, "nonexistent")
			Expect(err).ToNot(HaveOccurred())
		})

		It("Delete all repositorys in org", func() {
			otherOrgId, _ := uuid.NewUUID()
			err := store.Repository().DeleteAll(ctx, otherOrgId)
			Expect(err).ToNot(HaveOccurred())

			listParams := ListParams{Limit: 1000}
			repositorys, err := store.Repository().List(ctx, orgId, listParams)
			Expect(err).ToNot(HaveOccurred())
			Expect(len(repositorys.Items)).To(Equal(numRepositories))

			err = store.Repository().DeleteAll(ctx, orgId)
			Expect(err).ToNot(HaveOccurred())
		...
		It("List with paging", func() {
			listParams := ListParams{Limit: 1000}
			allRepositories, err := store.Repository().List(ctx, orgId, listParams)
			Expect(err).ToNot(HaveOccurred())
			...
			cont, err := ParseContinueString(repositorys.Metadata.Continue)
			Expect(err).ToNot(HaveOccurred())
