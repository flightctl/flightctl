...
func (s *DeviceStore) List(ctx context.Context, orgId uuid.UUID, listParams ListParams) (*api.DeviceList, error) {
	...
	query, err := BuildBaseListQuery(s.db.Model(&devices), orgId, listParams)
	if err != nil {
		return nil, err
	}
	if listParams.Limit > 0 {
	...
	if listParams.Limit > 0 && len(devices) > listParams.Limit {
		...
		if listParams.Continue != nil {
			...
			countQuery, err := BuildBaseListQuery(s.db.Model(&devices), orgId, listParams)
			if err != nil {
				return nil, err
			}
...
func (s *DeviceStore) Summary(ctx context.Context, orgId uuid.UUID, listParams ListParams) (*api.DevicesSummary, error) {
	query, err := BuildBaseListQuery(s.db.Model(&model.DeviceList{}), orgId, listParams)
	if err != nil {
		return nil, err
	}
