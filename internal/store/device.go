...
type DeviceStore struct {
	...
	genericStore *GenericStore[*model.Device, model.Device, api.Device, *model.DeviceList, api.DeviceList]
}
...
func NewDevice(db *gorm.DB, log logrus.FieldLogger) Device {
	genericStore := NewGenericStore[*model.Device, model.Device, api.Device, *model.DeviceList, api.DeviceList](
		db,
		log,
		model.NewDeviceFromApiResource,
		(*model.Device).ToApiResource,
		(*model.DeviceList).ToApiResource,
		model.DevicePtrReturnSelf,
		model.DevicePtrReturnSelf,
	)
...
func (s *DeviceStore) Delete(ctx context.Context, orgId uuid.UUID, name string, callback DeviceStoreCallback) error {
	return s.genericStore.Delete(
		...
		callback,
		Resource{Table: "enrollmentrequests", OrgID: orgId.String(), Name: name})
}
