...
type RepositoryStore struct {
	...
	genericStore *GenericStore[*model.Repository, model.Repository, api.Repository, *model.RepositoryList, api.RepositoryList]
}

type RepositoryStoreCallback func(*model.Repository, *model.Repository)
...
func NewRepository(db *gorm.DB, log logrus.FieldLogger) Repository {
	genericStore := NewGenericStore[*model.Repository, model.Repository, api.Repository, *model.RepositoryList, api.RepositoryList](
		db,
		log,
		model.NewRepositoryFromApiResource,
		(*model.Repository).ToApiResource,
		(*model.RepositoryList).ToApiResource,
		model.RepositoryPtrReturnSelf,
		model.RepositoryPtrReturnSelf,
	)
...
func (s *RepositoryStore) GetFleetRefs(ctx context.Context, orgId uuid.UUID, name string) (*api.FleetList, error) {
	...
	fleetList, _ := fleets.ToApiResource(nil, nil)
...
func (s *RepositoryStore) GetDeviceRefs(ctx context.Context, orgId uuid.UUID, name string) (*api.DeviceList, error) {
	...
	deviceList, _ := devices.ToApiResource(nil, nil)
