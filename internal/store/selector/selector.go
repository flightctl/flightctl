...
func (sr *selectorFieldResolver) ResolveNames(field SelectorFieldName) (map[SelectorFieldName]string, error) {
	...
	for fn, selectorField := range resolvedFields {
		fields[fn] = string(selectorField.Name)
	}
	return fields, nil
}
...
func (sr *selectorFieldResolver) ResolveFields(field SelectorFieldName) (map[SelectorFieldName]*SelectorField, error) {
	resolve := func(fn SelectorFieldName) (*SelectorField, error) {
		if resolvedField, exists := sr.schemaFields[fn]; exists {
			...
			return &SelectorField{
				Name:        SelectorFieldName(resolvedField.DBName),
				Type:        fieldType,
				...
				if schemaField.DataType == "jsonb" && schemaField.DBName == fieldParts[0] {
					...
					if strings.Contains(jsonbField, "::") {
						...
						if fieldType, ok := castTypeResolution[suffix]; ok {
							return &SelectorField{
								Name:        SelectorFieldName(fieldName),
								Type:        fieldType,
	...
	if resolvedField != nil {
		return map[SelectorFieldName]*SelectorField{field: resolvedField}, nil
	}

	if sr.fieldResolver != nil {
		refs := sr.fieldResolver.ResolveFieldName(field)
		if len(refs) > 0 {
			fields := make(map[SelectorFieldName]*SelectorField, len(refs))
