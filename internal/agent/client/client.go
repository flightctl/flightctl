...
func NewFromConfigWithHTTPClient(config *baseclient.Config, httpClient *http.Client, deviceNotFoundCallback DeviceNotFoundCallback) (*client.ClientWithResponses, error) {
	...
	wrappedClient := &DeviceNotFoundHTTPClient{
		...
		log:      log.NewPrefixLogger("http-client"),
	}
...
func (c *DeviceNotFoundHTTPClient) Do(req *http.Request) (*http.Response, error) {
	...
	backoffConfig := wait.Backoff{
		...
		Steps:    math.MaxInt32, // Never stop retrying
		Cap:      5 * time.Minute,
	}
	...
	err := wait.ExponentialBackoffWithContext(req.Context(), backoffConfig, func(ctx context.Context) (bool, error) {
		attemptCount++
		...
		if err != nil {
			...
			c.log.WithFields(map[string]interface{}{
				...
				"url":     req.URL.String(),
				"method":  req.Method,
			}).Warn("HTTP request failed with network error, will retry")
			return false, nil // Return false, nil to continue retrying
		}
		...
		if resp.StatusCode >= 500 && resp.StatusCode < 600 {
			...
			err := resp.Body.Close()
			if err != nil {
				lastError = fmt.Errorf("server error: status %d, close error: %v", resp.StatusCode, err)
				c.log.WithFields(map[string]interface{}{
					"attempt":     attemptCount,
					...
					"url":         req.URL.String(),
					"method":      req.Method,
				}).Warn("HTTP request failed with server error, will retry")
				return false, nil // Return false, nil to continue retrying
			}
			lastError = fmt.Errorf("server error: status %d", resp.StatusCode)
			c.log.WithFields(map[string]interface{}{
				"attempt":     attemptCount,
				"status_code": resp.StatusCode,
				"url":         req.URL.String(),
	...
	// If we get here and there was an error, it means we exhausted all retries
	if err != nil {
		// Use the last error we encountered, or the context error
		if lastError != nil {
			finalErr = lastError
		} else {
			finalErr = err
