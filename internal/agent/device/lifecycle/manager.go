...
// WipeCertificateAndRestartService wipes only the device certificate (keeping private key/CSR) and restarts the flightctl-agent service.
// This is used when the device is not found on the server but we want to preserve the key for re-enrollment.
func (m *LifecycleManager) WipeCertificateAndRestartService(ctx context.Context) error {
