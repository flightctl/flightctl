...
func NewWriterForUser(uid, gid int) *writer {
	return &writer{
		uid: uid,
		gid: gid,
	}
...
func (w *writer) OpenFile(path string, flag int, perm fs.FileMode) (*os.File, error) {
	f, err := os.OpenFile(w.PathFor(path), flag, perm)
	if err != nil {
...
func UnpackTar(writer Writer, tarPath, destDir string) error {
	...
	for {
		...
		case tar.TypeReg:
			...
			destFile, err := writer.OpenFile(destPath, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, perm)
			if err != nil {
