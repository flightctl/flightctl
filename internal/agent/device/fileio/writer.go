...
func (w *writer) copyFile(src, dst string) error {
	...
	fmt.Println("Copying file from", src, "to", dstTarget)

	fmt.Printf("Creating destination file: %v", srcFile)
	...
	fmt.Printf("Copied %d bytes from %v to %v", written, src, dstTarget)
