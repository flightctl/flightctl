...
func (b *BootcChecker) checkBootcTimer(ctx context.Context) v1beta1.Condition {
	...
	// Empty output means systemctl itself did not run successfully (binary missing,
	// broken systemd, etc.) — do not treat as non-compliant.
	if state == "" {
		return v1beta1.Condition{
			Type:    v1beta1.ConditionTypeDeviceBootcTimerCompliant,
			Status:  v1beta1.ConditionStatusFalse,
			Reason:  "CheckFailed",
			Message: fmt.Sprintf("failed to determine bootc timer state (exitCode=%d, stderr=%s)", exitCode, strings.TrimSpace(stderr)),
		}
	}
