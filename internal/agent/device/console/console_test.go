...
func sessionMetadata(term string, initialDimensions *v1alpha1.TerminalSize, command *v1alpha1.DeviceCommand, tty bool) string {
...
func TestConsole(t *testing.T) {
	...
	t.Run("create process and close without tty", func(t *testing.T) {
		...
		consoleDef := deviceConsole(sessionID, sessionMetadata("xterm", nil,
	...
	t.Run("separate stdout and stderr without tty", func(t *testing.T) {
		v := setupVars(t)
		sessionID := uuid.New().String()
		consoleDef := deviceConsole(sessionID, sessionMetadata("xterm", nil, &v1alpha1.DeviceCommand{
			Command: "echo",
			Args: []string{"stdout",
				";",
				"echo",
	...
	t.Run("echo stdin with tty, close stdin", func(t *testing.T) {
		v := setupVars(t)
		sessionID := uuid.New().String()
		consoleDef := deviceConsole(sessionID, sessionMetadata("xterm", nil, nil, true))
