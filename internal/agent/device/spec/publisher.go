...
const (
	...
	// minPollDelay paces successful / no-content polls so a fast response
	// does not immediately hammer /rendered.
	minPollDelay = 5 * time.Second

	errorBaseDelay     = 5 * time.Second
	errorBackoffFactor = 2.0
	maxErrorPollDelay  = 5 * time.Minute
	errorJitterFactor  = 0.2
)
...
func defaultErrorBackoff() poll.Config {
	return poll.Config{
		BaseDelay:    errorBaseDelay,
		Factor:       errorBackoffFactor,
		MaxDelay:     maxErrorPollDelay,
		JitterFactor: errorJitterFactor,
	}
}
...
func (n *publisher) Run(ctx context.Context) {
	...
	for {
		...
		if err != nil {
			...
			errorTries = 0
			if elapsed < minPollDelay {
				delay = minPollDelay - elapsed
