...
func (p *quadletProvider) collectOCITargets(ctx context.Context, configProvider client.PullConfigProvider) (dependency.OCIPullTargetsByUser, error) {
...
func (p *quadletProvider) extractNestedTargets(ctx context.Context, configProvider client.PullConfigProvider) (*AppData, error) {
	if !p.isImageBased() {
		return &AppData{}, nil
	}
...
func (p *quadletProvider) parentIsAvailable(ctx context.Context) (ref string, digest string, available bool, err error) {
	if !p.isImageBased() {
		return "", "", true, nil
	}
...
func quadletAppPath(appName string, user v1beta1.Username) (string, error) {
	if user.IsCurrentProcessUser() || user.IsRootUser() {
		return filepath.Join(lifecycle.RootfulQuadletAppPath, appName), nil
	} else {
