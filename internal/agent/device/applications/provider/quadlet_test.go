...
func TestDefaultServiceName(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := defaultServiceName(tt.basename, tt.ext)
			require.Equal(t, tt.expected, result)
		})
	}
}
...
`),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := getServiceName(rw, tt.filePath, tt.ext, tt.defaultName)
			if tt.expectError {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
`),
			...
			checkFileContents: map[string]func(*testing.T, []byte){
				...
				"myapp-flightctl.target": func(t *testing.T, content []byte) {
				...
				"myapp-.container.d/99-flightctl.conf": func(t *testing.T, content []byte) {
					contentStr := string(content)
					require.Contains(t, contentStr, "[Unit]")
					require.Contains(t, contentStr, "PartOf=myapp-flightctl.target")
...
`),
			...
			checkFileContents: map[string]func(*testing.T, []byte){
				...
				"myapp-.container.d/99-flightctl.conf": func(t *testing.T, content []byte) {
					require.Contains(t, string(content), "PartOf=myapp-flightctl.target")
				},
			},
		},
		{
