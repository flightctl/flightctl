...
type appProvider interface {
	...
	collectOCITargets(ctx context.Context, configProvider client.PullConfigProvider) (dependency.OCIPullTargetsByUser, error)
	...
	extractNestedTargets(ctx context.Context, configProvider client.PullConfigProvider) (*AppData, error)
...
// WithPullConfigProvider sets the pull configuration provider for OCI operations.
func WithPullConfigProvider(p client.PullConfigProvider) CollectOpt {
	...
	for _, p := range providers {
		...
		if err := p.EnsureDependencies(ctx); err != nil {
			...
			if errors.Is(err, errors.ErrAppDependency) {
				depsErr = errors.Join(depsErr, fmt.Errorf("app %s: %w", p.Name(), err))
				continue
			}
			return nil, fmt.Errorf("%w: %w", errors.ErrAppProviders, err)
		}
		...
		if err != nil {
			return nil, fmt.Errorf("collecting nested targets for %s: %w", p.Name(), err)
		}
		if requeue {
			needsRequeue = true
		}
...
func collectNestedForProvider(
	...
	if appData != nil && len(appData.Targets) > 0 {
...
func ExtractNestedTargetsFromImage(
	...
	return p.extractNestedTargets(ctx, configProvider)
}
...
func FromDeviceSpec(
	...
	return iter.Map(providers, func(p *appProvider) Provider {
		return *p
	}), nil
}
...
func ensureDependenciesFromAppType(deps []dependencyBins, checker commandChecker) error {
	...
	for _, dep := range deps {
		if !slices.ContainsFunc(dep.variants, checker) {
			errs = append(errs, fmt.Errorf("dependencies missing one of: %v", dep.variants))
		}
	}

	if len(errs) > 0 {
		return fmt.Errorf("%w: %w", errors.ErrAppDependency, errors.Join(errs...))
	}
	return nil
}
