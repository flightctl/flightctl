...
func (p *composeProvider) collectOCITargets(ctx context.Context, configProvider client.PullConfigProvider) (dependency.OCIPullTargetsByUser, error) {
...
func (p *composeProvider) extractNestedTargets(ctx context.Context, configProvider client.PullConfigProvider) (*AppData, error) {
	if !p.isImageBased() {
		return &AppData{}, nil
	}
...
func (p *composeProvider) parentIsAvailable(ctx context.Context) (ref string, digest string, available bool, err error) {
	if !p.isImageBased() {
		return "", "", true, nil
	}
