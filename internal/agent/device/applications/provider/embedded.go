...
func newEmbeddedHandler(appType v1beta1.AppType, name string, rw fileio.ReadWriter, bootTime string, installed bool) (embeddedAppTypeHandler, error) {
	switch appType {
...
func newEmbedded(log *log.PrefixLogger, podman *client.Podman, readWriter fileio.ReadWriter, name string, appType v1beta1.AppType, bootTime string, installed bool) (Provider, error) {
	handler, err := newEmbeddedHandler(appType, name, readWriter, bootTime, installed)
	if err != nil {
