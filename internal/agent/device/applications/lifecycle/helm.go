...
const (
	...
	helmDefaultNamespacePrefix = "flightctl-"
...
// helmVersionConfig holds the configuration for post-renderer operations.
type helmVersionConfig struct {
	options func(appID string) []client.HelmOption
	cleanup func()
}
...
func (h *HelmHandler) Execute(ctx context.Context, actions Actions) error {
	...
	if err != nil {
		return fmt.Errorf("setting up version config: %w", err)
	}
	...
	for _, action := range actions {
		switch action.Type {
		case ActionAdd:
			if err := h.add(ctx, versionConfig, &action); err != nil {
				return err
			}
		case ActionRemove:
		...
		case ActionUpdate:
			if err := h.update(ctx, versionConfig, &action); err != nil {
				return err
			}
		default:
...
func (h *HelmHandler) add(ctx context.Context, versionConfig *helmVersionConfig, action *Action) error {
	...
	opts = append(opts, versionConfig.options(action.ID)...)
	if h.kubeconfigPath != "" {
...
func (h *HelmHandler) update(ctx context.Context, versionConfig *helmVersionConfig, action *Action) error {
	releaseName := action.Name
	chartPath := action.Path
	helmSpec := h.getHelmSpec(action)
	namespace := helmSpec.Namespace
...
func (h *HelmHandler) getHelmSpec(action *Action) HelmSpec {
	...
	if spec.Namespace == "" {
		spec.Namespace = fmt.Sprintf("%s%s", helmDefaultNamespacePrefix, action.Name)
	}
	return spec
}
