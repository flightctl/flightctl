...
const (
	...
	EmbeddedComposeAppPath = "/use/local/lib/compose/manifests"
)

var _ ActionHandler = (*Compose)(nil)

type Compose struct {
	client *client.Compose
	podman *client.Podman
	log    *log.PrefixLogger
}

func NewCompose(log *log.PrefixLogger, client *client.Compose, podman *client.Podman) *Compose {
	return &Compose{
		client: client,
		podman: podman,
		log:    log,
...
func (c *Compose) add(ctx context.Context, action *Action) error {
	...
	return c.client.Up(ctx, appPath)
}
...
func (c *Compose) remove(ctx context.Context, action *Action) error {
	...
	// remove volumes
	if err := c.podman.RemoveVolumes(ctx, labels); err != nil {
		return err
	}
...
func (c *Compose) update(ctx context.Context, action *Action) error {
	...
	// change to workdir and run `docker-compose up -d`
	return c.client.UpFromWorkDir(ctx, appPath)
}
