	...
	testCases := []struct {
		...
		},
		{
			...
			events: []client.PodmanEvent{
				...
				mockPodmanEventStopped("app1", v1beta1.CurrentProcessUsername, "app1-service-2", "died", 0, 15),
			},
			expectedReady:   "1/2",
			expectedStatus:  v1beta1.ApplicationStatusRunning,
		...
		{
			name: "single app all containers completed successfully",
			apps: []Application{
				createTestApplication(require, "app1", v1beta1.ApplicationStatusPreparing, v1beta1.CurrentProcessUsername),
			},
			events: []client.PodmanEvent{
				mockPodmanEventSuccess("app1", v1beta1.CurrentProcessUsername, "app1-service-1", "start"),
				mockPodmanEventSuccess("app1", v1beta1.CurrentProcessUsername, "app1-service-2", "start"),
				mockPodmanEventSuccess("app1", v1beta1.CurrentProcessUsername, "app1-service-1", "died"), // exit 0, no signal
				mockPodmanEventSuccess("app1", v1beta1.CurrentProcessUsername, "app1-service-2", "died"), // exit 0, no signal
			},
			expectedReady:   "0/2",
...
func mockPodmanEventSuccess(name string, username v1beta1.Username, service, status string) client.PodmanEvent {
	return createMockPodmanEvent(name, username, service, status, 0, 0)
}

func mockPodmanEventError(name string, username v1beta1.Username, service, status string, exitCode int) client.PodmanEvent {
	return createMockPodmanEvent(name, username, service, status, exitCode, 0)
}

func mockPodmanEventStopped(name string, username v1beta1.Username, service, status string, exitCode int, signal int) client.PodmanEvent {
	return createMockPodmanEvent(name, username, service, status, exitCode, signal)
}

func createMockPodmanEvent(name string, username v1beta1.Username, service, status string, exitCode int, signal int) client.PodmanEvent {
