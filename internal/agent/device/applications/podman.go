...
func NewPodmanMonitor(log *log.PrefixLogger, client *client.Podman) *PodmanMonitor {
...
func (m *PodmanMonitor) Run(ctx context.Context) error {
	...
	m.cmd = m.client.EventsCmd(ctx, []string{"start", "die"}) // TODO: extend
...
func (m *PodmanMonitor) ExecuteActions(ctx context.Context) error {
	...
	for i := range actions {
		if actions[i].Handler == lifecycle.ActionHandlerCompose {
			if err := m.compose.Execute(ctx, &actions[i]); err != nil {
				return err
			}
		}
	}
...
// drainActions returns the current actions and resets the actions to nil. this
// also avoids holding the lock for a long time.
...
func (m *PodmanMonitor) Status() []v1alpha1.ApplicationStatus {
	m.mu.Lock()
	defer m.mu.Unlock()

	status := make([]v1alpha1.ApplicationStatus, 0, len(m.apps))
