...
// newComposeID generates a new unique id for a compose application. The id is generated by
// sanitizing the input and appending a random suffix.
func newComposeID(input string) string {
	...
	for i := range suffix {
		// random number generator not used for security purposes
		//nolint:gosec
		suffix[i] = charset[rand.Intn(len(charset))]
	}

	maxLength := v1alpha1.MaxDNSNameLength - len(suffix) - 1
	if len(id) > maxLength {
		// truncate the id to fit within the max length this is ok because the
