...
const (
	// metricsAddrDefault pins the agent’s loopback Prometheus endpoint.
	metricsAddrDefault = "127.0.0.1:15690"
	// pprofPortDefault pins the agent’s loopback pprof port.
...
func (m *metricsServer) Run(ctx context.Context) {
	...
	if err := m.srv.Run(ctx, instmetrics.WithListenAddr(metricsAddrDefault)); err != nil {
		if m.log != nil {
...
func (p *pprofServer) Run(ctx context.Context) {
	...
	if err := p.srv.Run(ctx, instpprof.WithPort(pprofPortDefault)); err != nil {
		if p.log != nil {
