...
func (b *BusType) Initialize(ctx context.Context, kvStore kvstore.KVStore, provider queues.Provider, log logrus.FieldLogger) error {
	vm, err := newVersionManager(ctx, kvStore, provider, log)
	if err != nil {
		return err
	}
	_ = b.GetOrInit(vm)
...
func newVersionManager(ctx context.Context, kvStore kvstore.KVStore, provider queues.Provider, log logrus.FieldLogger) (*VersionManager, error) {
