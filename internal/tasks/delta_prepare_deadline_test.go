...
func (f *fakeDeadlineStore) CASPrepareStatus(_ context.Context, id uuid.UUID, to string) error {
	...
	for i := range f.waiting {
		...
		if f.waiting[i].Status != model.DeltaPrepareWaiting && f.waiting[i].Status != model.DeltaPrepareFailing {
...
func TestDeltaPrepareDeadlinePoll(t *testing.T) {
	...
	t.Run("When a waiting prepare is past deadline it should CAS-fail and emit FleetRolloutStarted", func(t *testing.T) {
		...
		tvSvc.EXPECT().GetLatestTemplateVersion(gomock.Any(), orgId, "fleet-1").Return(&domain.TemplateVersion{Metadata: domain.ObjectMeta{Name: lo.ToPtr("tv-1")}}, domain.StatusOK()).AnyTimes()
	...
	t.Run("When a device prepare is past deadline it should clear device status and emit completion", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		deviceSvc := deviceservice.NewMockService(ctrl)
		generation := int64(7)
	...
	t.Run("When the failure claim loses its CAS it should not emit", func(t *testing.T) {
		rec := &deadlineEventRecorder{}
		prep := model.DeltaPrepare{ID: uuid.New(), OrgID: orgId, Kind: domain.DeviceKind, Name: "device-1", Status: model.DeltaPrepareWaiting}
		store := &fakeDeadlineStore{waiting: []model.DeltaPrepare{prep}, casErr: flterrors.ErrNoRowsUpdated}
		task := &DeltaPrepareDeadline{log: log, deltaStore: store, eventSvc: rec}

		task.Poll(context.Background())
		assert.Equal(t, model.DeltaPrepareWaiting, store.waiting[0].Status)
		assert.Empty(t, rec.events)
	})
}
