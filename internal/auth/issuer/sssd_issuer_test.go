...
func NewMockSSSDAuthenticator(authenticateFunc func(username, password string) error, mockUser *user.User, groups []string) SSSDAuthenticator {
...
func TestNewSSSDOIDCProvider(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			provider, err := NewSSSDOIDCProvider(tt.caClient, tt.cfg)
...
func TestSSSDOIDCProvider_Token(t *testing.T) {
	...
	caClient := createTestCAClient(t)
	cfg := &config.SSSDOIDCIssuer{}
	provider, err := NewSSSDOIDCProvider(caClient, cfg)
	require.NoError(t, err)
...
func TestSSSDOIDCProvider_AuthorizationCodeFlow(t *testing.T) {
	// Test the authorization code flow with mocked SSSD authentication
	mockUser := &user.User{
		Uid:      "1000",
		Gid:      "1000",
...
func TestSSSDOIDCProvider_RefreshTokenFlow(t *testing.T) {
	// Test the refresh token flow with mocked SSSD authentication
	mockUser := &user.User{
		Uid:      "1000",
		Gid:      "1000",
		Username: testUsername,
	...
	t.Run("refresh_token_with_updated_user_info", func(t *testing.T) {
		// Test that refresh token flow updates user information from SSSD
		...
		updatedMockAuth := NewMockSSSDAuthenticator(
			func(username, password string) error {
				return nil
			},
			mockUser,
		...
		updatedProvider, err := NewSSSDOIDCProviderWithAuthenticator(caClient, cfg, updatedMockAuth)
		require.NoError(t, err)
