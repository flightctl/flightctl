...
var requests []TestRequest = []TestRequest{
	...
	{
		url:      "https://fctl.io/ws/v1/devices/foo/console",
		method:   http.MethodGet,
...
func TestAuthN(t *testing.T) {
	...
	testCases := []struct {
		...
		{"valid", true, true, nil, nil, http.StatusOK},
		{"invalid", false, true, nil, nil, http.StatusUnauthorized},
		{"tokenErr", true, true, fmt.Errorf("auth error"), nil, http.StatusUnauthorized},
		{"identityErr", true, true, nil, fmt.Errorf("auth error"), http.StatusOK},
		{"noToken", true, false, nil, nil, http.StatusUnauthorized},
	}
	...
	for _, r := range authNRequests {
		for _, tc := range testCases {
			...
			if r.shouldSkipCheck || !tc.withToken {
				...
				authNMock.EXPECT().ValidateToken(gomock.Any(), gomock.Any()).Return(tc.tokenValid, tc.tokenErr).Times(1)
				if tc.tokenErr != nil || !tc.tokenValid {
