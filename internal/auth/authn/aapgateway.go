...
func (a AapGatewayAuth) ValidateToken(ctx context.Context, token string) (bool, error) {
	if strings.HasPrefix(token, "Bearer ") {
		// oauth2 token
		req, err := http.NewRequestWithContext(ctx, http.MethodGet, fmt.Sprintf("%s/v1/me/", a.GatewayUrl), nil)
		if err != nil {
			return false, err
		}
		...
		_, err := jwt.Parse([]byte(token), jwt.WithKey(jwa.RS256, a.PublicKey))
		return err == nil, err
	}
}
...
func (AapGatewayAuth) GetAuthToken(r *http.Request) (string, bool) {
	jwtHeader := r.Header[AAP_GATEWAY_AUTH_HEADER]
	if len(jwtHeader) == 0 {
		return common.GetAuthToken(r)
	}
	return jwtHeader[0], true
}
