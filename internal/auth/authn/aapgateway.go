...
func NewAapGatewayAuth(gatewayUrl string, externalGatewayUrl string, clientTlsConfig *tls.Config) AapGatewayAuth {
	authN := AapGatewayAuth{
		...
		cache:              ttlcache.New[string, *AAPUser](ttlcache.WithTTL[string, *AAPUser](time.Minute)),
	}
...
func (a AapGatewayAuth) loadUserInfo(token string) (*AAPUser, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to read response body: %v", res.StatusCode)
	}
	...
	a.cache.Set(token, &userInfo.Results[0], 5*time.Second)
