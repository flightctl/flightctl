...
type deviceStatusStore interface {
	...
	UpdateStatus(ctx context.Context, orgId uuid.UUID, device *domain.Device, previous *domain.Device) (*domain.Device, *domain.Device, error)
}
...
func (s *storePreparingStatus) setFleet(ctx context.Context, orgId uuid.UUID, name string, completed, total int) error {
	...
	fleet.Status.DeltaGeneration = &domain.DeltaGenerationStatus{Completed: int64(completed), Total: int64(total)}
...
func (s *storePreparingStatus) setDevice(ctx context.Context, orgId uuid.UUID, name string, completed, total int) error {
	...
	device.Status.DeltaGeneration = &domain.DeltaGenerationStatus{Completed: int64(completed), Total: int64(total)}
	_, _, err = s.devices.UpdateStatus(ctx, orgId, device, nil)
	return err
}
