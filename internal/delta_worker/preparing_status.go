...
func (s *storePreparingStatus) SetProgress(ctx context.Context, orgId uuid.UUID, kind, name string, progress tasks.GenerationProgress) error {
	switch kind {
	case domain.FleetKind:
...
func (s *storePreparingStatus) setFleetProgress(ctx context.Context, orgId uuid.UUID, name string, progress tasks.GenerationProgress) error {
	if s.fleets == nil {
		return fmt.Errorf("fleet store is required")
	}
	fleet, err := s.fleets.Get(ctx, orgId, name)
...
func (s *storePreparingStatus) setDeviceProgress(ctx context.Context, orgId uuid.UUID, name string, progress tasks.GenerationProgress) error {
	if s.devices == nil {
		return fmt.Errorf("device store is required")
	}
	device, err := s.devices.Get(ctx, orgId, name)
...
func applyGenerationProgress(dst *domain.DeltaGenerationStatus, p tasks.GenerationProgress) {
	dst.Phase = nil
	dst.Percent = nil
	dst.BytesDone = nil
	dst.BytesTotal = nil
