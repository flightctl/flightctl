...
// GetRepositoryAccessibleEvent creates an event for repotester task
func GetRepositoryAccessibleEvent(ctx context.Context, repoName string, err error) *api.Event {
...
func GetResourceSyncTaskEvent(ctx context.Context, repoName string, commitHash string, totalChanges int, totalErrors int, updatedFields []api.ResourceUpdatedDetailsUpdatedFields, log logrus.FieldLogger) *api.Event {
	...
	return getBaseEvent(ctx, resourceEvent{
		...
		OutcomeSuccess: fmt.Sprintf("Processed %d changes for commit %s", totalChanges, commitHash),
		OutcomeFailure: func() string {
			return fmt.Sprintf("Processed %d changes for commit %s (%d failures)", totalChanges, commitHash, totalErrors)
		},
		Status:        statusFunc(),
