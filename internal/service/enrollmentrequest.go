...
func (h *ServiceHandler) CreateEnrollmentRequest(ctx context.Context, request server.CreateEnrollmentRequestRequestObject) (server.CreateEnrollmentRequestResponseObject, error) {
	...
	if err != nil {
		return server.CreateEnrollmentRequest401JSONResponse{Message: fmt.Sprintf("auth failed: %v", err)}, nil
	}
	if !allowed {
		return server.CreateEnrollmentRequest403JSONResponse{Message: "cannot create enrollment request"}, nil
	}
...
func (h *ServiceHandler) ListEnrollmentRequests(ctx context.Context, request server.ListEnrollmentRequestsRequestObject) (server.ListEnrollmentRequestsResponseObject, error) {
	...
	if err != nil {
		return server.ListEnrollmentRequests401JSONResponse{Message: fmt.Sprintf("auth failed: %v", err)}, nil
	}
	if !allowed {
		return server.ListEnrollmentRequests403JSONResponse{Message: "cannot list enrollment requests"}, nil
	}
...
func (h *ServiceHandler) DeleteEnrollmentRequests(ctx context.Context, request server.DeleteEnrollmentRequestsRequestObject) (server.DeleteEnrollmentRequestsResponseObject, error) {
	...
	if err != nil {
		return server.DeleteEnrollmentRequests401JSONResponse{Message: fmt.Sprintf("auth failed: %v", err)}, nil
	}
	if !allowed {
		return server.DeleteEnrollmentRequests403JSONResponse{Message: "cannot delete enrollment requests"}, nil
	}
	orgId := store.NullOrgId
...
func (h *ServiceHandler) ReadEnrollmentRequest(ctx context.Context, request server.ReadEnrollmentRequestRequestObject) (server.ReadEnrollmentRequestResponseObject, error) {
	allowed, err := auth.GetAuthZ().CheckPermission(ctx, "enrollmentrequests", "read")
	if err != nil {
		return server.ReadEnrollmentRequest401JSONResponse{Message: fmt.Sprintf("auth failed: %v", err)}, nil
	}
	if !allowed {
		return server.ReadEnrollmentRequest403JSONResponse{Message: "cannot read enrollment request"}, nil
	}
...
func (h *ServiceHandler) ReplaceEnrollmentRequest(ctx context.Context, request server.ReplaceEnrollmentRequestRequestObject) (server.ReplaceEnrollmentRequestResponseObject, error) {
	allowed, err := auth.GetAuthZ().CheckPermission(ctx, "enrollmentrequests", "replace")
	if err != nil {
		return server.ReplaceEnrollmentRequest401JSONResponse{Message: fmt.Sprintf("auth failed: %v", err)}, nil
	}
	if !allowed {
		return server.ReplaceEnrollmentRequest403JSONResponse{Message: "cannot replace enrollment request"}, nil
	}
	orgId := store.NullOrgId
...
func (h *ServiceHandler) DeleteEnrollmentRequest(ctx context.Context, request server.DeleteEnrollmentRequestRequestObject) (server.DeleteEnrollmentRequestResponseObject, error) {
	...
	if err != nil {
		return server.DeleteEnrollmentRequest401JSONResponse{Message: fmt.Sprintf("auth failed: %v", err)}, nil
	}
	if !allowed {
		return server.DeleteEnrollmentRequest403JSONResponse{Message: "cannot delete enrollment request"}, nil
	}
	orgId := store.NullOrgId
...
func (h *ServiceHandler) ReadEnrollmentRequestStatus(ctx context.Context, request server.ReadEnrollmentRequestStatusRequestObject) (server.ReadEnrollmentRequestStatusResponseObject, error) {
	allowed, err := auth.GetAuthZ().CheckPermission(ctx, "enrollmentrequests", "readstatus")
	if err != nil {
		return server.ReadEnrollmentRequestStatus401JSONResponse{Message: fmt.Sprintf("auth failed: %v", err)}, nil
	}
	if !allowed {
		return server.ReadEnrollmentRequestStatus403JSONResponse{Message: "cannot read enrollment request status"}, nil
	}
	orgId := store.NullOrgId
...
func (h *ServiceHandler) ApproveEnrollmentRequest(ctx context.Context, request server.ApproveEnrollmentRequestRequestObject) (server.ApproveEnrollmentRequestResponseObject, error) {
	allowed, err := auth.GetAuthZ().CheckPermission(ctx, "enrollmentrequests", "approve")
	if err != nil {
		return server.ApproveEnrollmentRequest401JSONResponse{Message: fmt.Sprintf("auth failed: %v", err)}, nil
	}
	if !allowed {
		return server.ApproveEnrollmentRequest403JSONResponse{Message: "cannot approve enrollment request"}, nil
	}
	orgId := store.NullOrgId
...
func (h *ServiceHandler) ReplaceEnrollmentRequestStatus(ctx context.Context, request server.ReplaceEnrollmentRequestStatusRequestObject) (server.ReplaceEnrollmentRequestStatusResponseObject, error) {
	allowed, err := auth.GetAuthZ().CheckPermission(ctx, "enrollmentrequests", "replacestatus")
	if err != nil {
		return server.ReplaceEnrollmentRequestStatus401JSONResponse{Message: fmt.Sprintf("auth failed: %v", err)}, nil
	}
	if !allowed {
		return server.ReplaceEnrollmentRequestStatus403JSONResponse{Message: "cannot replace enrollment request status"}, nil
	}
	orgId := store.NullOrgId
