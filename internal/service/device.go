...
func (h *ServiceHandler) UpdateToOutOfDateByOwner(ctx context.Context, owner string) error {
	return h.store.Device().UpdateToOutOfDateByOwner(ctx, getOrgIdFromContext(ctx), owner)
}
...
func (h *ServiceHandler) UpdateServerSideDeviceStatus(ctx context.Context, name string) error {
	...
	if common.UpdateServiceSideStatus(ctx, orgId, device, h.store, h.log) {
