...
func TestWrapWithTracing(t *testing.T) {
	...
	t.Run("When inner is non-nil it should delegate calls and return the result unchanged", func(t *testing.T) {
		...
		_, status := traced.GetFleet(context.Background(), uuid.New(), "missing", domain.GetFleetParams{})
		require.Equal(t, domain.StatusResourceNotFound(domain.FleetKind, "missing"), status)
	})
}
