...
func mappedIdentityToUserInfoResponse(mappedIdentity *identity.MappedIdentity) *api.UserInfoResponse {
	...
	if len(mappedIdentity.Organizations) > 0 {
		...
		for _, org := range mappedIdentity.Organizations {
			orgs = append(orgs, orgModelToAPI(org))
		}
...
// orgModelToAPI converts an orgmodel.Organization to api.Organization
func orgModelToAPI(org *orgmodel.Organization) api.Organization {
	name := org.ID.String()
