...
func TestCreateCatalog(t *testing.T) {
	...
	t.Run("When managed metadata fields are set by the caller CreateCatalogFromUntrusted should clear them before creation", func(t *testing.T) {
		...
		require.Equal(t, int64(1), lo.FromPtr(fakeStore.catalogs[catalogKey(orgId, "c4")].Metadata.Generation))
	})
	...
	t.Run("When managed metadata fields are set by the caller CreateCatalog (trusted) should preserve owner and set generation to 1", func(t *testing.T) {
		...
		require.Equal(t, int64(1), lo.FromPtr(fakeStore.catalogs[catalogKey(orgId, "c4-trusted")].Metadata.Generation))
	})
}
...
func TestReplaceCatalog(t *testing.T) {
	...
	t.Run("When managed metadata fields are set by the caller ReplaceCatalogFromUntrusted should clear them before replacing", func(t *testing.T) {
		...
		require.Equal(t, int64(1), lo.FromPtr(fakeStore.catalogs[catalogKey(orgId, "replace-untrusted")].Metadata.Generation))
	})
	...
	t.Run("When managed metadata fields are set by the caller ReplaceCatalog (trusted) should preserve owner and set generation to 1 on create", func(t *testing.T) {
		...
		require.Equal(t, int64(1), lo.FromPtr(fakeStore.catalogs[catalogKey(orgId, "replace-trusted")].Metadata.Generation))
	})
	...
	t.Run("When replacing with a changed spec it should bump generation", func(t *testing.T) {
		...
		fakeStore.catalogs[catalogKey(orgId, "gen-bump")] = &existing
		...
		require.Equal(t, int32(http.StatusOK), status.Code)
		require.Equal(t, int64(3), lo.FromPtr(fakeStore.catalogs[catalogKey(orgId, "gen-bump")].Metadata.Generation))
	})
	...
	t.Run("When replacing with the same spec it should keep generation", func(t *testing.T) {
		...
		existing.Metadata.Generation = lo.ToPtr(int64(2))
		fakeStore.catalogs[catalogKey(orgId, "gen-same")] = &existing

		updated := createTestCatalog("gen-same", nil)

		_, status := h.ReplaceCatalog(context.Background(), orgId, "gen-same", updated, true)
		require.Equal(t, int32(http.StatusOK), status.Code)
		require.Equal(t, int64(2), lo.FromPtr(fakeStore.catalogs[catalogKey(orgId, "gen-same")].Metadata.Generation))
	})
}
