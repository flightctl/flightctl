...
func (h *ServiceHandler) ReplaceCertificateSigningRequest(ctx context.Context, request server.ReplaceCertificateSigningRequestRequestObject) (server.ReplaceCertificateSigningRequestResponseObject, error) {
	...
	case nil:
		...
		if created && request.Body.Spec.SignerName == "enrollment" {
			approveReq := server.UpdateCertificateSigningRequestApprovalRequestObject{
				Name: *request.Body.Metadata.Name,
			}
			approveResp, _ := h.UpdateCertificateSigningRequestApproval(ctx, approveReq)
			_, ok := approveResp.(server.UpdateCertificateSigningRequestApproval200JSONResponse)
			if !ok {
				c := []api.Condition{{
...
func (h *ServiceHandler) UpdateCertificateSigningRequestApproval(ctx context.Context, request server.UpdateCertificateSigningRequestApprovalRequestObject) (server.UpdateCertificateSigningRequestApprovalResponseObject, error) {
	...
	// If the CSR is approved now, sign the certificate.
	// TODO: This needs to become async.
	if api.IsStatusConditionTrue(newCSR.Status.Conditions, api.CertificateSigningRequestApproved) {
		signedCert, err := signApprovedCertificateSigningRequest(h.ca, *newCSR)
		if err != nil {
			switch {
		...
		newCSR.Status.Certificate = &signedCert
	}

	_, err = h.store.CertificateSigningRequest().UpdateStatus(ctx, orgId, newCSR)
	switch err {
	case nil:
		return server.UpdateCertificateSigningRequestApproval200JSONResponse{}, nil
	case flterrors.ErrResourceNotFound:
