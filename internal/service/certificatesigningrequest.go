...
func (h *ServiceHandler) ReplaceCertificateSigningRequest(ctx context.Context, request server.ReplaceCertificateSigningRequestRequestObject) (server.ReplaceCertificateSigningRequestResponseObject, error) {
	...
	case nil:
		if request.Body.Spec.SignerName == "enrollment" {
			...
			if !ok {
				// add failed status if newly created enrollment CSR could not be approved;
				// do not update status on pre-existing enrollment CSR with same name
				if created {
					c := []api.Condition{{
					...
					if err != nil {
						return nil, fmt.Errorf("enrollment CSR for %s could not be auto-approved: %s\nstatus condition could not be set to 'failed': %s", request.Name, approveResp, err)
					}
				}
