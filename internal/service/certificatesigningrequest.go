...
func (h *ServiceHandler) CreateCertificateSigningRequest(ctx context.Context, csr api.CertificateSigningRequest) (*api.CertificateSigningRequest, api.Status) {
	...
	if csr.Spec.SignerName == "ca" {
		return nil, api.StatusBadRequest("the signer name 'ca' is deprecated and no longer supported; please specify a valid signer name")
	}
	...
	signer := h.ca.GetSigner(csr.Spec.SignerName)
	if signer == nil {
		return nil, api.StatusBadRequest(fmt.Sprintf("singer %q not found", csr.Spec.SignerName))
	}
...
func (h *ServiceHandler) PatchCertificateSigningRequest(ctx context.Context, name string, patch api.PatchRequest) (*api.CertificateSigningRequest, api.Status) {
	...
	if signer == nil {
		return nil, api.StatusBadRequest(fmt.Sprintf("singer %q not found", newObj.Spec.SignerName))
	}
