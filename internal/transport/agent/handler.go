...
func ValidateDeviceAccessFromContext(ctx context.Context, name string, ca *crypto.CAClient, log logrus.FieldLogger) (string, error) {
	...
	if !ok || cn == "" {
		log.WithField("device", name).Warn("attempt to access device without TLS common name")
	...
	if err != nil {
		log.WithField("cn", cn).Warn("invalid TLS common name format for device")
		return "", errors.New("invalid TLS common name format")
	}

	// If a specific device name is expected, check it matches the CN-derived fingerprint
	if name != "" {
		expectedCN, err := ca.CNFromDeviceFingerprint(name)
		if err != nil {
			log.WithError(err).WithField("device", name).Error("failed to calculate expected CN from device name")
			return "", err
		}
		if cn != expectedCN {
			log.WithFields(logrus.Fields{
				...
				"actual":   cn,
				"device":   name,
			}).Warn("TLS CN does not match expected CN for device access")
			return "", errors.New("invalid TLS common name for device")
		}
	}

	log.WithField("device", name).Debugf("device access validated using CN fingerprint: %s", fingerprint)
