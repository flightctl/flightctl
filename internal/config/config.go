...
type PAMOIDCIssuer struct {
	...
	// BrandingDir is the path to a drop-in directory containing branding assets
	// (CSS overrides, logos, favicon). All files are optional; missing files fall back
	// to embedded defaults. The server reads this directory at startup only.
	// Default: "/etc/flightctl/branding"
	BrandingDir string `json:"brandingDir,omitempty"`
}
...
func applyPAMOIDCIssuerDefaults(c *Config) {
	...
	if c.Auth.PAMOIDCIssuer.DisplayName == "" {
		c.Auth.PAMOIDCIssuer.DisplayName = "Flight Control"
	}
	if c.Auth.PAMOIDCIssuer.BrandingDir == "" {
		c.Auth.PAMOIDCIssuer.BrandingDir = "/etc/flightctl/branding"
	}
}
