...
// This function is called by the console session manager to signal that they are ready to accept connections
...
func (s *AgentGrpcServer) Stream(stream pb.RouterService_StreamServer) error {
	...
	if loaded {
		...
		if otherSideStream != nil {
			...
			err := s.forwardGrpcOnlyStream(ctx, stream, otherSideStream, sessionId, actual)
			return err
		}
		...
		if otherSideSession != nil {
			...
			err := s.forwardConsoleSession(ctx, stream, otherSideSession, sessionId, actual)
			return err
		}
...
func (s *AgentGrpcServer) forwardConsoleSession(ctx context.Context, stream pb.RouterService_StreamServer, otherSideSession *console.ConsoleSession, sessionId string, actual any) error {
	err := s.forwardChannels(ctx, stream, otherSideSession)
...
func (s *AgentGrpcServer) forwardChannels(ctx context.Context, a pb.RouterService_StreamServer, b *console.ConsoleSession) error {
	...
	g.Go(func() error { return s.pipeStreamToChannel(a, b.RecvCh) })
	g.Go(func() error { return s.pipeChannelToStream(b.SendCh, a) })
	return g.Wait()
}

func (s *AgentGrpcServer) pipeStreamToChannel(a pb.RouterService_StreamServer, ch chan []byte) error {
	s.log.Infof("pipeStreamToChannel START")
	defer close(ch)
...
func (s *AgentGrpcServer) pipeChannelToStream(ch chan []byte, a pb.RouterService_StreamServer) error {
	for {
		payload, ok := <-ch
		if !ok {
			_ = a.Send(&pb.StreamResponse{
				Payload: []byte{},
				Closed:  true,
			})
			return io.EOF
