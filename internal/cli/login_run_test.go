...
func TestLoginRun_ListOrganizationsApiError(t *testing.T) {
	require := require.New(t)

	// Create a mock HTTP client that returns an error for organizations.
	mockClient := &http.Client{
		Transport: &mockRoundTripper{
			roundTrip: func(req *http.Request) (*http.Response, error) {
				var resp *http.Response
	...
	// Check for the expected error message.
	require.Error(t, err)
	expectedError := "failed to list organizations: status code 500"
	require.Equal(t, expectedError, err.Error())
}

func TestLoginRun_ListOrganizationsInvalidResponse(t *testing.T) {
	require := require.New(t)

	// Create a mock HTTP client that returns an invalid response for organizations.
	mockClient := &http.Client{
		Transport: &mockRoundTripper{
			roundTrip: func(req *http.Request) (*http.Response, error) {
				var resp *http.Response
	...
	// Check for the expected error message.
	require.Error(t, err)
	require.Contains(t, err.Error(), "failed to list organizations:")
}
