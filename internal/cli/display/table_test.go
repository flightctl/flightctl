	...
	tests := []struct {
	...
	}{
		{
			...
			mutate: func(s *api.DeviceStatus) {
				s.Updated.Size = lo.ToPtr("45 MiB")
			},
			wantContains: []string{"45 MiB"},
			wantHeader:   []string{"NAME", "ALIAS", "OWNER", "SYSTEM", "UPDATED", "APPLICATIONS"},
		},
		{
			name: "When lastUpdateFallbackReason is set it should show (fallback) in list and single table",
			mutate: func(s *api.DeviceStatus) {
				s.Os.LastUpdateFallbackReason = lo.ToPtr("delta apply failed: apply error")
			},
		...
		},
		{
			...
			mutate: func(s *api.DeviceStatus) {
				s.Os.LastUpdateFallbackReason = lo.ToPtr("")
			},
			wantAbsent: []string{"(fallback)"},
		},
	}
...
func TestTableFormatter_WhenDeltaEligibleIsSetItShouldNotPrintItOnDeviceTable(t *testing.T) {
	...
	device := upToDateDevice("dev-1")
	device.Status.Capabilities = &api.DeviceCapabilities{DeltaEligible: lo.ToPtr(true)}
...
func TestTableFormatter_WhenWideItShouldKeepLabelsLastAndPutSizeFallbackInUpdated(t *testing.T) {
	...
	device := upToDateDevice("dev-1")
	device.Status.Updated.Size = lo.ToPtr("45 MiB")
	device.Status.Os.LastUpdateFallbackReason = lo.ToPtr("delta apply failed")
