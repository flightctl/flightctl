...
func TestEditOptions_Complete(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "uses KUBE_EDITOR when no flag",
			editorFlag:     "",
			kubeEditor:     "vim",
			editor:         "emacs",
			expectedEditor: "vim",
		...
		{
			name:           "uses EDITOR when no flag or KUBE_EDITOR",
			editorFlag:     "",
			kubeEditor:     "",
			editor:         "emacs",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			// Set up environment
			oldKubeEditor := os.Getenv("KUBE_EDITOR")
			oldEditor := os.Getenv("EDITOR")
			defer func() {
				os.Setenv("KUBE_EDITOR", oldKubeEditor)
				os.Setenv("EDITOR", oldEditor)
			}()

			os.Setenv("KUBE_EDITOR", tc.kubeEditor)
...
func TestEditOptions_resourceToYAML(t *testing.T) {
	...
	opts := DefaultEditOptions()
	yamlBytes, err := opts.resourceToFormat(response)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
