...
func TestAPIMetadataRegistryContainsResumeDevices(t *testing.T) {
	...
	if metadata.Resource != "device/resume" {
		t.Errorf("ResumeDevices resource = %v, want device/resume", metadata.Resource)
	}
	if metadata.Action != "resume" {
		t.Errorf("ResumeDevices action = %v, want resume", metadata.Action)
	}
}
