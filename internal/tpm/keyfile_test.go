...
func TestGenerateTPM2KeyFile(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "valid importable key",
			keyType:     ImportableKey,
			parent:      0x81000001,
			publicData:  template,
			privateData: tpm2.TPM2BPrivate{Buffer: []byte("mock-private-key-data")},
			opts:        nil,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Verify basic fields
			require.Equal(t, tt.parent, forceUint32(key.Parent), "parent handle mismatch")
			...
			case ImportableKey:
				expectedOID = oidImportableKey
			case SealedKey:
				expectedOID = oidSealedKey
			}
...
func TestKeyFileTypeConstants(t *testing.T) {
	tests := []struct {
		...
		{ImportableKey, oidImportableKey},
		{SealedKey, oidSealedKey},
	}
...
func TestPEMFormat(t *testing.T) {
	...
	result, err := GenerateTPM2KeyFile(LoadableKey, parentHandle.Handle.HandleValue(), ldevidResp.OutPublic, ldevidResp.OutPrivate)
	require.NoError(t, err, "unexpected error")
