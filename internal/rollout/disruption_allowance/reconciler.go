...
func (r *reconciler) getFleetCounts(ctx context.Context, orgId uuid.UUID, fleet *api.Fleet) (map[string]*Counts, error) {
	...
	totalCounts, err := r.store.Device().CountByLabels(ctx, orgId, store.ListParams{
		FieldSelector: selector.NewFieldSelectorFromMapOrDie(map[string]string{"metadata.owner": util.ResourceOwner(api.FleetKind, lo.FromPtr(fleet.Metadata.Name))}, false),
	}, groupBy, false)
	if err != nil {
		return nil, err
	}
...
func (r *reconciler) Reconcile(ctx context.Context) {
	...
	if err != nil {
		r.log.WithError(err).Error("Failed to query disruption allowance fleets")
		return
	}
