...
func newBatchSequenceSelector(sequence v1alpha1.BatchSequence, store store.Store, orgId uuid.UUID, fleet *v1alpha1.Fleet, templateVersionName string, log logrus.FieldLogger) RolloutDeviceSelector {
...
func (b *batchSelection) IsComplete(ctx context.Context) (bool, error) {
	...
	// A device is counted as completed if it has completed successfully or, it is not disconnected, and it is in error state
