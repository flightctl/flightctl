...
var HealthChecks util.Singleton[Healthchecker]

func NewHealthchecker(store store.Store, log logrus.FieldLogger) *Healthchecker {
...
func (h *Healthchecker) work(ctx context.Context) {
	...
	for {
		...
		for orgId := range pending {
			h.save(context.Background(), orgId, pending[orgId])
		}
		pending = make(map[uuid.UUID][]string)
...
func (h *Healthchecker) Add(ctx context.Context, orgId uuid.UUID, name string) error {
	...
	default:
		h.log.Warnf("Healthcheck channel is full, dropping request for %s in org %s", name, orgId)
