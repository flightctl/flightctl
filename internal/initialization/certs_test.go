...
func TestServerCertificates(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			setup: func(cfg *config.Config) {
				os.WriteFile(filepath.Join(cfg.Service.CertStore, cfg.Service.ServerCertName+".crt"), []byte("invalid"), 0600)
			},
			expectedErr: ErrReadingDefaultCerts,
		},
		{
			name:        "default certs do not exist and are generated",
			setup:       func(cfg *config.Config) {},
			expectedErr: nil,
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			ca, serverCerts, err := ServerCertificates(ctx, cfg, log)
