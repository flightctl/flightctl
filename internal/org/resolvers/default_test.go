...
func TestDefaultResolver_getDefaultOrg(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			go orgCache.Start()
			defer orgCache.Stop()
			resolver := NewDefaultResolver(test.mockOrgStore, orgCache)
			if test.setOrgInCache {
				resolver.cache.Set(test.orgID, defaultOrg)
			}
...
func TestDefaultResolver_GetUserOrganizations(t *testing.T) {
	...
	resolver := NewDefaultResolver(nil, orgCache)
	resolver.cache.Set(org.DefaultID, &model.Organization{
		ID:          org.DefaultID,
		DisplayName: "Default",
	})
	orgs, err := resolver.GetUserOrganizations(ctx, identity)
	assert.NoError(t, err)
	assert.Equal(t, []*model.Organization{resolver.cache.Get(org.DefaultID)}, orgs)
}
