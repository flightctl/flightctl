...
func setupOrgInCache(resolver *ExternalResolver, id uuid.UUID, org *model.Organization) {
	resolver.cache.Set(id, org)
}
...
func TestExternalResolver_getOrg(t *testing.T) {
	...
	testDifferentExternalID := "different-external-id"
	...
	tests := []struct {
	...
	}{
		{
			...
			setupCache: func(r *ExternalResolver) {
				setupOrgInCache(r, testOrgID, testOrg)
			},
		...
		},
		{
			...
			setupCache: func(r *ExternalResolver) {
				...
				setupOrgInCache(r, differentID, testOrg)
			},
			store: &mockOrgStore{
				getByIDResult: &model.Organization{
					ID:          testOrgID,
					ExternalID:  testDifferentExternalID,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			orgCache := cache.NewOrganizationTTL(tt.ttl)
			go orgCache.Start()
			defer orgCache.Stop()

			resolver := NewExternalResolver(tt.store, orgCache, nil, logrus.New())
...
func TestExternalResolver_GetUserOrganizations(t *testing.T) {
	...
	testExternalIDOne := "test-external-id-one"
	testExternalIDTwo := "test-external-id-two"
	...
	test := []struct {
		...
		},
		{
			...
			store: &mockOrgStore{
				listByExternalIDsResult: []*model.Organization{
					{ID: orgID, ExternalID: testExternalIDOne, DisplayName: "Test Org 1"},
					{ID: orgID2, ExternalID: testExternalIDTwo, DisplayName: "Test Org 2"},
				},
			},
			externalOrgProvider: &mockExternalOrgProvider{
				orgs: []org.ExternalOrganization{
					{ID: testExternalIDOne, Name: "Test Org 1"},
					{ID: testExternalIDTwo, Name: "Test Org 2"},
				},
			},
	...
	for _, tt := range test {
		t.Run(tt.name, func(t *testing.T) {
			orgCache := cache.NewOrganizationTTL(time.Hour)
			go orgCache.Start()
			defer orgCache.Stop()
			resolver := NewExternalResolver(tt.store, orgCache, tt.externalOrgProvider, logrus.New())
