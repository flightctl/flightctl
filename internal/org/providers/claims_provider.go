...
func (c *ClaimsProvider) IsMemberOf(ctx context.Context, identity common.Identity, id string) (bool, error) {
	orgs, err := claimsFromIdentity(identity)
	if err != nil {
		return false, err
	}

	if _, ok := orgs[orgID(id)]; ok {
		return true, nil
	}
